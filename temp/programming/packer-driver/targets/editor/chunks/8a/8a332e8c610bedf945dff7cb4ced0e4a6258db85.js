      ...
      _export("PoolManager", PoolManager = (_dec5 = ccclass('PoolManager'), _dec6 = property({
        ...
        _initPools() {
          ...
          for (const item of this.objectPoolItems) {
            ...
            const pool = new NodePool(); // 预先创建指定数量的对象

            for (let i = 0; i < item.poolInitSize; i++) {
              const node = instantiate(item.prefab);
              const component = node.getComponent(_crd && PoolObjectBase === void 0 ? (_reportPossibleCrUseOfPoolObjectBase({
                error: Error()
        ...
        _resetNodeStatus(node) {
          const poolObjectBase = node.getComponent(_crd && PoolObjectBase === void 0 ? (_reportPossibleCrUseOfPoolObjectBase({
            error: Error()
          }), PoolObjectBase) : PoolObjectBase);
