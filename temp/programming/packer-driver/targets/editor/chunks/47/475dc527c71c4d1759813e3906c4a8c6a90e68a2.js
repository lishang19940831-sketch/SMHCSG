    ...
    execute: function () {
      ...
      _export("UnlockItem", UnlockItem = (_dec5 = ccclass('UnlockItem'), _dec6 = property({
        ...
        gotoNextStage() {
          ...
          const config = this.getCurrentStageConfig(); // 重置当前阶段的资源需求
        ...
        reserveGold(amount) {
          ...
          const displayRemainGold = this.getDisplayRemainGold(); // 如果预留的资源足够解锁，可以在这里触发相关事件
        ...
        onSetUnlockStatue(data) {
          ...
          if (isCurrentStage || isAnyStage) {
            let oldState = this.unlockState; // 如果指定了阶段索引，跳转到该阶段
            ...
            const config = this.getCurrentStageConfig(); // 只有当 cost 存在时才更新当前阶段的成本
        ...
        CostGold(cost) {
          ...
          const currentCost = this.getCurrentStageCost(); // 清除原本tween
          ...
          tween(this.goldIcon).to(0.01, {
            ...
            if (this.remainGold <= 0 && this.unlockState === (_crd && BuildUnlockState === void 0 ? (_reportPossibleCrUseOfBuildUnlockState({
              ...
              const currentItemType = this.ItemType; // 停止node缩放动画并恢复原始scale
        ...
        OnCost(pickupComponent, wpos, duration = 0) {
        ...
        ShowUnlockTip() {
          ...
          const squareNode = this.node.getChildByName('square'); // 保存原始颜色和缩放

          const originalScale = squareNode.scale.clone(); // 设置呼吸动画 - 缩放效果
