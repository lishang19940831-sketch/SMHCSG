    ...
    execute: function () {
      ...
      _export("CameraFollow", CameraFollow = (_dec = ccclass('CameraFollow'), _dec2 = property({
        ...
        lateUpdate(deltaTime) {
          ...
          const targetWorld = this.target.worldPosition;
          const targetPosition = new Vec3(targetWorld.x + this.initialOffset.x + this.shakeOffset.x, targetWorld.y + this.initialOffset.y + this.shakeOffset.y, targetWorld.z + this.initialOffset.z + this.shakeOffset.z);

          if (this.smoothFollow) {
            // 平滑跟随（线性插值）
            const currentPos = this.node.worldPosition;
            const newPos = new Vec3(currentPos.x + (targetPosition.x - currentPos.x) * this.followSpeed, currentPos.y + (targetPosition.y - currentPos.y) * this.followSpeed, currentPos.z + (targetPosition.z - currentPos.z) * this.followSpeed);
        ...
        snapToTarget() {
          ...
          const targetWorld = this.target.worldPosition;
          const targetPosition = new Vec3(targetWorld.x + this.initialOffset.x, targetWorld.y + this.initialOffset.y, targetWorld.z + this.initialOffset.z);
          this.node.setWorldPosition(targetPosition);
          this.cameraParticleNode && this.cameraParticleNode.setWorldPosition(targetPosition);
        }
        /**
        ...
        onBossSpawn(target = null) {
          ...
          const targetHeight = this.normalOrthoHeight + this.bossOrthoHeightIncrease; // 创建缓动对象
        ...
        onBossSpawn2(target = null, duration = 1.2) {
          // 计算目标正交高度
          const targetHeight = this.normalOrthoHeight; // 创建缓动对象
        ...
        moveTo(worldPos, duration = 1.0, onComplete, delay = 0) {
        ...
        moveToTarget(targetNode, duration = 1.0, onComplete, delay = 0) {
          ...
          const offsetPos = new Vec3(targetWorld.x + this.initialOffset.x, targetWorld.y + this.initialOffset.y, targetWorld.z + this.initialOffset.z);
        ...
        bossSpawn2(target = null, duration = this.zoomDuration) {
        ...
        startShake(strength) {
          this.isShaking = true;
          this.shakeTimer = 0;
          const shakeStr = strength !== undefined ? strength : this.bossAttackShakeStrength; //console.log(`[CameraFollow]  摄像机抖动开始，强度: ${shakeStr}`);
        }
        /**
         * 更新抖动效果
         */
        ...
        updateShake(deltaTime) {
          ...
          const currentStrength = this.bossAttackShakeStrength * decay; // 生成随机抖动偏移（使用高频随机）
