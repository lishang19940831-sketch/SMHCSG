    ...
    execute: function () {
      ...
      _export("TreeManager", TreeManager = (_dec = ccclass('TreeManager'), _dec2 = property({
        ...
        findTreesInNode(node, listType) {
          // 检查当前节点是否有Tree组件
          const treeComponent = node.getComponent(_crd && Tree === void 0 ? (_reportPossibleCrUseOfTree({
            error: Error()
          }), Tree) : Tree);
        ...
        addTree(tree, listType = TreeListType.LIST_1) {
          ...
          const listName = listType === TreeListType.LIST_1 ? '列表1' : '列表2'; //console.log(`TreeManager: 添加树木 ${tree.node.name} 到${listName}, 该列表总数: ${targetList.length}`);
        }
        /**
         * 获取指定范围内的树木
        ...
        getRangeTrees(centerPos, range, listType = TreeListType.LIST_1) {
          const treesInRange = [];
        ...
        getRangeTreesWithDistance(centerPos, range, listType = TreeListType.LIST_1) {
          const treesWithDistance = [];
          const rangeSquared = range * range;
          const targetList = this.getTreeList(listType);
        ...
        getNearestTree(centerPos, maxRange, listType = TreeListType.LIST_1) {
