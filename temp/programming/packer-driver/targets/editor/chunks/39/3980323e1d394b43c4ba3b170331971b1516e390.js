      ...
      _export("WheatCrop", WheatCrop = (_dec = ccclass('WheatCrop'), _dec2 = property({
        ...
        _playFragmentEffect() {
          ...
          for (let i = 0; i < this.fragmentCount; i++) {
            const fragment = instantiate(meshNodes[0]); // 脱离原节点树，挂到同一父节点下独立运动
            ...
            const targetPos = new Vec3(centerPos.x + Math.cos(angle) * dist, centerPos.y + this.fragmentUpHeight * (0.8 + Math.random() * 0.4), centerPos.z + Math.sin(angle) * dist); // 第1段：飞散 + 上升（抛物线感）

            tween(fragment).to(this.fragmentDuration, {
              worldPosition: targetPos
            }, {
              easing: 'quadOut'
            }) // 第2段：缩小消失
            .to(this.fragmentDuration * 0.4, {
            ...
            this.scheduleOnce(() => {
              if (fragment && fragment.isValid) {
                ...
                tween(fragment).to(this.fragmentDuration * 0.45, {
