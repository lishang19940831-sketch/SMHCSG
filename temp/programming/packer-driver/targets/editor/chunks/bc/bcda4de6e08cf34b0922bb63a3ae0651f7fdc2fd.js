    ...
    execute: function () {
      ...
      _export("CoinContainer", CoinContainer = (_dec = ccclass('CoinContainer'), _dec2 = property({
          ...
          if (this.unlimitedMode) {
            ...
            if (shouldRemoveVisualItem) {
              ...
              if (outerItems.length > 0) {
                const visualItem = outerItems[0]; // 生成临时金币
            ...
            if (coinItem) {
              const dropItemCom = coinItem.getComponent(_crd && DropItemCom === void 0 ? (_reportPossibleCrUseOfDropItemCom({
                error: Error()
              ...
              // 兜底
              const tempCoin = manager.pool.getNode((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
                error: Error()
              }), ObjectType) : ObjectType).DropItemCoin);

              if (tempCoin) {
                const dropItemCom = tempCoin.getComponent(_crd && DropItemCom === void 0 ? (_reportPossibleCrUseOfDropItemCom({
                  error: Error()
                }), DropItemCom) : DropItemCom);
        ...
        receiveCoin(fromPosition) {
          // 获取金币对象
          const coin = manager.pool.getNode((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
            error: Error()
          }), ObjectType) : ObjectType).DropItemCoin);
