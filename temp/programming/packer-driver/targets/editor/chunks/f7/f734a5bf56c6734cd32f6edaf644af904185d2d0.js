      ...
      _export("HealthBar_Bar", HealthBar_Bar = (_dec = ccclass('HealthBar_Bar'), _dec2 = property({
        ...
        update(dt) {
          ...
          if (this.isAnimating) {
            ...
            const progress = Math.min(1.0, this.animationTimer / this.animationDuration); // 如果背景填充存在，平滑更新其宽度

            if (this.backgroundFill && this.bgFillSprite) {
              // 计算当前应该显示的血量百分比（线性插值）
              const currentWidth = this.originalWidth * (this.currentHealthPercent - (this.currentHealthPercent - this.targetHealthPercent) * progress);
          ...
          if (uiTransform) {
            ...
            if (this.originalWidth <= 0) {
              if (currentWidth > 0) {
                ...
                const spriteWidth = this.barSprite.spriteFrame.rect.width;
        ...
        updateHealth(percentage) {
          ...
          const healthPercent = Math.max(0, Math.min(1, percentage)); // 更新填充区域宽度（立即更新）
