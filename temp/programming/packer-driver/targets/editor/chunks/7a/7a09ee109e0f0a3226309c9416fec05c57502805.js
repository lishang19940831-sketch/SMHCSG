  ...
  return {
    ...
    execute: function () {
      ...
      AudioMgr = class AudioMgr {
        ...
        async preloadAudios(paths) {
          return;
          const promises = paths.map(async path => {
            try {
              const clip = await (_crd && resMgr === void 0 ? (_reportPossibleCrUseOfresMgr({
                error: Error()
        ...
        async playMusic(path, loop = true, volume = 1.0) {
          ...
          if (this.audioCache.has(path)) {
            ...
            clip = await (_crd && resMgr === void 0 ? (_reportPossibleCrUseOfresMgr({
              error: Error()
            }), resMgr) : resMgr).loadRes(path);
            this.audioCache.set(path, clip);
          }
        ...
        async playEffect(path, volume = 1.0, ignoreCooldown = false) {
          if (!this.effectSwitch) return; // 检查防重复播放

          if (!ignoreCooldown && this.isEffectInCooldown(path)) {
          ...
          this.effectPlayRecord.set(path, Date.now());
        ...
        isEffectInCooldown(path) {
          const lastPlayTime = this.effectPlayRecord.get(path);
        ...
        getDataBoolean(key, defaultValue = false) {
          const value = (_crd && dataMgr === void 0 ? (_reportPossibleCrUseOfdataMgr({
            error: Error()
