  ...
  return {
    ...
    execute: function () {
      ...
      _export("ShopCommon", ShopCommon = (_dec = ccclass('ShopCommon'), _dec2 = property({
        ...
        onUse(pickupComponent, duration = 0) {
        ...
        handleConsumedItem(item) {
          ...
          const tarWorldRot = this.itemLayout.node.getWorldRotation(); // item.setWorldRotationFromEuler(0, 0, 0);
        ...
        moveCustomerAlongPath(customer, pathIndex = 0) {
        ...
        checkWaitSolder() {
          ...
          if (itemCount > 0 && this.waitList.length > 0) {
            const customer = this.waitList[0]; // 检查顾客是否需要当前商品类型
        ...
        AddWaitSolder(node, immediate = false) {
          const customer = node.getComponent(_crd && Customer === void 0 ? (_reportPossibleCrUseOfCustomer({
            error: Error()
          }), Customer) : Customer);
        ...
        updateCustomerSpawn(dt) {
          ...
          if (this.spawnTimer >= this.SPAWN_INTERVAL) {
            ...
            const waitingCustomers = this.waitList.length; // 生成逻辑：
        ...
        spawnCustomer(immediate = false) {
          // 双重检查：生成前再次确认
          const waitingCustomers = this.waitList.length; // 检查是否超过上限
          ...
          let customerNode = null; // 首先尝试从对象池获取
          ...
          if (customerNode) {
            ...
            const customer = customerNode.getComponent(_crd && Customer === void 0 ? (_reportPossibleCrUseOfCustomer({
              error: Error()
            }), Customer) : Customer);
