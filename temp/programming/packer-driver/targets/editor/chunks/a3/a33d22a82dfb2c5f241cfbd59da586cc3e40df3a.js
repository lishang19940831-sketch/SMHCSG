    ...
    execute: function () {
      ...
      _export("ArrowTower", ArrowTower = (_dec = ccclass('ArrowTower'), _dec2 = property({
        ...
        _fireArrow() {
          ...
          const arrowNode = manager.pool.getNode((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
            error: Error()
          }), ObjectType) : ObjectType).Arrow, this.node.parent);
          if (!arrowNode) return; // 获取发射点世界坐标

          const startPos = this.firePoint ? this.firePoint.getWorldPosition() : this.node.getWorldPosition(); // 计算朝向目标的方向向量
          ...
          const arrow = arrowNode.getComponent(_crd && Arrow === void 0 ? (_reportPossibleCrUseOfArrow({
            error: Error()
          }), Arrow) : Arrow);
