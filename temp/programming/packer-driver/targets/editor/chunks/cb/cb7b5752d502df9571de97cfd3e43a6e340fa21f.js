    ...
    execute: function () {
      ...
      _export("ModelAnimationComponent", ModelAnimationComponent = (_dec = ccclass('ModelAnimationComponent'), _dec2 = property({
        ...
        onAnimationComplete() {
          ...
          const currentAnimName = ((_this$currentAnimatio = this.currentAnimationData) == null ? void 0 : _this$currentAnimatio.name) || ''; // 发送动画完成事件
        ...
        updateFaceDirection(directionX, rotationSmoothness = -1) {
        ...
        updateFaceDirectionFrom3D(direction, rotationSmoothness = -1) {
          if (Vec3.equals(direction, Vec3.ZERO)) return; // 使用3D旋转工具类来处理朝向，传入当前的帧时间

          const deltaTime = game.deltaTime;
          const horizontalDirection = (_crd && Rotation3DUtils === void 0 ? (_reportPossibleCrUseOfRotation3DUtils({
            error: Error()
        ...
        playAttack(timeScale, isLoop = false) {
        ...
        playRunAttack(timeScale, isLoop = false) {
        ...
        playAnimation(name, loop, timeScale = 1, crossFade = 0.1) {
          if (!this.animation) {
        ...
        getAnimationDuration(animName) {
          ...
          if (animName) {
            const state = this.animation.getState(animName); // state.duration 已经是秒为单位的实际时长
        ...
        crossFadeAnimation(name, fadeTime = 0.3, loop = false, timeScale = 1) {
          if (!this.animation) {
            //console.error('SkeletalAnimation 组件未找到');
            return;
          } // 检查动画剪辑是否存在
        ...
        playAnimationFromProgress(name, loop, timeScale = 1, startProgress = 0, crossFade = 0.1) {
          if (!this.animation) return; // 检查动画是否存在

          const clip = this.animation.clips.find(clip => clip && clip.name === name);
