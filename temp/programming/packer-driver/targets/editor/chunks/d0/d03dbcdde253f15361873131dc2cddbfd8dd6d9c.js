    ...
    execute: function () {
      ...
      _export("Character", Character = (_dec = ccclass('Character'), _dec2 = property({
        ...
        startAttackAnimation() {
          ...
          const shouldPlayRunAttack = this.supportMoveAttack && isMoving; // 选择动画名称

          const animName = shouldPlayRunAttack ? this.animationComponent.RunAttackAnimName : this.animationComponent.AttackAnimName; // 计算攻击动画的timeScale

          const attackTimeScale = this.calculateAttackTimeScale(animName); // console.log(`[攻击动画] 开始播放攻击动画`, {
          //     '角色': this.node.name,
          //     '动画名称': animName,
          ...
          const eventId = shouldPlayRunAttack ? (_crd && FrameEventId === void 0 ? (_reportPossibleCrUseOfFrameEventId({
            error: Error()
          ...
          this.scheduleOnce(() => {
            ...
            const progress = this.animationComponent.getCurrentAnimationProgress(); // console.log(`[攻击动画] 动画播放状态检查`, {
            //     '角色': this.node.name,
            //     '是否正在播放': isPlaying,
        ...
        handleAttackComplete() {
          ...
          const target = this.GetAttackTarget(); // 如果没有目标，直接退出攻击状态
        ...
        handleMoveAttackAnimation() {
          ...
          const isPlayingAttack = currentAnimName === this.animationComponent.AttackAnimName; // 如果移动状态发生变化，切换动画
        ...
        switchToRunAttack() {
          // 获取当前动画进度
          const currentProgress = this.animationComponent.getCurrentAnimationProgress(); // 计算新动画的timeScale

          const attackTimeScale = this.calculateAttackTimeScale(this.animationComponent.RunAttackAnimName); // 清除旧的帧事件
        ...
        switchToStandAttack() {
          ...
          const attackTimeScale = this.calculateAttackTimeScale(this.animationComponent.AttackAnimName); // 清除旧的帧事件
        ...
        onMoveStateUpdate(isMoving) {
          const currentState = this.stateComponent.getCurrentState(); // 死亡和技能状态不响应移动状态变化
         ...
         */
        attack() {
          const currentState = this.stateComponent.getCurrentState(); // 如果角色已死亡或正在释放技能，不执行攻击
