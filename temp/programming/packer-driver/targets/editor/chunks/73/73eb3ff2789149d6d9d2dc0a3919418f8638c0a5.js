      ...
      _export("HealthComponent", HealthComponent = (_dec = ccclass('HealthComponent'), _dec2 = property({
        ...
        takeDamage(damageData) {
          ...
          if (this.isDead || this.isImmune || this.invincible) {
            const reason = this.isDead ? '已死亡' : this.isImmune ? '免疫中' : '无敌状态'; // console.log(`[目标受击] ❌ 伤害被阻挡: ${reason}`, {
            //     '目标': this.node.name,
            //     'isDead': this.isDead,
          ...
          const beforeHp = this.currentHealth; // 减少生命值

          this.currentHealth = Math.max(0, this.currentHealth - damageData.damage);
          const actualDamage = beforeHp - this.currentHealth; // console.log(`[目标受击] ✓ 伤害生效`, {
          //     '目标': this.node.name,
