      ...
      _export("PickupComponent", PickupComponent = (_dec7 = ccclass('PickupComponent'), _dec8 = property({
        ...
        isItemPickable(itemType) {
          ...
          if (this.autoPickupWhenFull && this.maxTotalCount !== -1) {
            const currentCount = this.itemMap.get(itemType) || 0; // const flyingCount = this.flyingItemsCount.get(itemType) || 0;
          ...
          const isFull = this.checkIsFull(itemType); // 如果容器已满，检查是否允许超限后继续自动拾取
        ...
        checkAndPickupItems() {
          // 获取角色位置
          const position = this.node.getWorldPosition(); // 从DropManager获取范围内的物品（带距离信息）

          const nearbyItemsWithDistance = manager.drop.getRangeItemsWithDistance(position, this.pickupRange); // 检查是否有物品
        ...
        pickupItem(item, cb) {
          // 查找匹配的容器
          const matchingContainer = this.findMatchingContainer(item.objectType); // 必须有容器且容器允许拾取
          ...
          const itemNode = item.node; // 立即增加计数和播放音效
        ...
        arrangeContainers() {
          ...
          for (const container of this.itemContainer) {
            ...
            if (itemCount > 0) {
              ...
              const targetPos = new Vec3(currentPos.x, currentPos.y, zPosition); // 添加缓动动画
        ...
        addItemToContainer(item, container) {
          ...
          const yOffset = items.length * container.itemSpacing; // 从管理器中移除物品
        ...
        createFlyAnimation(itemNode, onComplete, isAddFlyingCount = true) {
          // 获取物品类型
          const itemType = this.getItemObjectType(itemNode); // 查找匹配的容器（必须存在）

          const matchingContainer = this.findMatchingContainer(itemType); // 判断容器是否已满
          ...
          const isContainerFull = totalCount >= matchingContainer.itemMaxCount; // 增加飞行中的物品计数
        ...
        getItemObjectType(itemNode) {
          const itemCom = itemNode.getComponent(_crd && DropItemCom === void 0 ? (_reportPossibleCrUseOfDropItemCom({
            error: Error()
        ...
        addItem(item, count = 1) {
          const type = item.objectType; // 获取当前物品类型的数量，如果不存在则初始化为0

          const currentCount = this.itemMap.get(type) || 0;
          const totalCount = currentCount + count; // 更新物品数量
        ...
        consumeItem(type, count) {
          ...
          // 获取当前物品总数
          const totalCount = this.getItemCount(type); // 找到匹配的容器

          const matchingContainer = this.findMatchingContainer(type); // 更新数量统计
          ...
          const consumedItems = []; // 如果有匹配的容器，处理物品显示

          if (matchingContainer && matchingContainer.itemNode) {
            // 获取容器中当前堆叠的物品数量
            const containerCount = this.containerItems.get(type) || []; // 计算容器外的物品数量
        ...
        createVirtualItemForConsume(type, container) {
          ...
          const containerCount = this.containerItems.get(type) || []; // 计算顶部位置

          const topYOffset = containerCount.length * container.itemSpacing; // 设置位置到容器顶部
        ...
        removeItemsFromContainer(type, count) {
          ...
          const removeList = items.splice(items.length - removeCount, removeCount); // 重新排列剩余物品
        ...
        async magnetPickup(magnetRange) {
          const range = magnetRange || this.pickupRange * 2;
          const position = this.node.getWorldPosition(); // 获取扩大范围内的物品（带距离信息）

          const itemsWithDistance = manager.drop.getRangeItemsWithDistance(position, range); // 如果没有物品，直接返回
          ...
          const pickableItems = itemsWithDistance.filter(itemData => {
            const container = this.findMatchingContainer(itemData.item.objectType); // 必须有对应容器且容器允许拾取
          ...
          for (const itemData of pickableItems) {
            ...
            const delay = Math.random() * 0.2; // 为每个物品创建Promise
            ...
            const itemPromise = new Promise(resolve => {
              tween(item.node).delay(delay).to(time, {
                ...
                // 处理物品拾取效果（更新统计）
                this.handlePickupEffect(item);
                const matchingContainer = this.findMatchingContainer(item.objectType); // 将物品添加到容器中（已经在过滤时确保了容器存在）
          ...
          await Promise.all(pickupPromises); //console.log('拾取完成');
        }
        /**
         * 设置指定类型容器的拾取状态
         * @param itemType 物品类型
