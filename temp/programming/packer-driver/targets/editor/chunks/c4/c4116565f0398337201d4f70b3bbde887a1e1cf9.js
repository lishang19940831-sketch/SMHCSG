      ...
      _export("AutoHuntAIComponent", AutoHuntAIComponent = (_dec = ccclass('AutoHuntAIComponent'), _dec2 = property({
        ...
        searchForEnemies() {
          ...
          for (let i = 0; i < targets.length; i++) {
            ...
            if (target && manager.game.isCanHunted(target.node)) {
              // 检查目标是否已死亡
              const healthComponent = target.node.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
                error: Error()
        ...
        generatePatrolPoint() {
          ...
          while (attempts < maxAttempts) {
            ...
            const distance = Math.random() * this.patrolRadius; // 计算巡逻点位置
        ...
        handleHuntDecision() {
          ...
          const healthComponent = this.currentTarget.node.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
            error: Error()
          }), HealthComponent) : HealthComponent);
          ...
          const distance = Vec3.distance(currentPos, targetPos); // 更新目标距离
        ...
        handleAttackDecision() {
          ...
          const attackRange = this.character.attackComponent.attackRangeValue; // 如果敌人离开攻击范围，继续追击
        ...
        handleReturnDecision() {
          const currentPos = this.node.getWorldPosition();
          const distance = Vec3.distance(currentPos, this._initialPosition); // 如果已经回到初始位置附近
