  ...
  return {
    ...
    execute: function () {
      ...
      _export("Rotation3DUtils", Rotation3DUtils = class Rotation3DUtils {
         ...
         */
        static faceDirection(node, direction, smoothness = 0, deltaTime) {
        ...
        static facePosition(node, targetPos, smoothness = 0, deltaTime) {
          if (!node) return;
        ...
        static faceNode(node, targetNode, smoothness = 0, deltaTime) {
        ...
        static updateYRotationFromX(node, directionX, smoothness = 0, deltaTime) {
        ...
        static calculateTimeFactor(smoothness, deltaTime) {
          ...
          const dt = deltaTime !== undefined ? deltaTime : game.deltaTime; // 基于时间的插值因子计算
        ...
        static smoothRotateToEuler(node, targetEuler, smoothness, deltaTime) {
          const currentEuler = node.eulerAngles; // 处理角度差异，确保选择最短路径

          let targetY = targetEuler.y;
          let currentY = currentEuler.y; // 处理角度跨越180度的情况
          ...
          const factor = this.calculateTimeFactor(smoothness, deltaTime); // 对每个轴进行插值

          const newEuler = new Vec3(math.lerp(currentEuler.x, targetEuler.x, factor), math.lerp(currentY, targetY, factor), math.lerp(currentEuler.z, targetEuler.z, factor)); // 确保Y轴角度在0-360范围内
        ...
        static isDirectionEqual(dir1, dir2, threshold = 0.01) {
