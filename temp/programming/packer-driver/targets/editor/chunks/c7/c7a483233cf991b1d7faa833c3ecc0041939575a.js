      ...
      _export("EnemyManager", EnemyManager = (_dec = ccclass('EnemyManager'), _dec2 = property({
        ...
        getBigEnemyFromPool() {
          ...
          if (bigEnemyNode) {
            ...
            const enemy = bigEnemyNode.getComponent(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
              error: Error()
            }), Enemy) : Enemy);
        ...
        checkTowerMode() {
          ...
          const arrowTower1Unlocked = ((_manager$game$unlockI = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
            error: Error()
          ...
          const arrowTower2Unlocked = ((_manager$game$unlockI2 = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
            error: Error()
        ...
        spawnEnemyDirectional(direction) {
          ...
          const wallGroup = direction === 'C' ? this.wallGroupA : this.wallGroupB; // 从对应方向的Box内随机生成位置
        ...
        _pickRoundRobinNode(direction, nodes) {
          ...
          const total = nodes.length; // 最多轮询一圈，找到第一个有效节点

          for (let i = 0; i < total; i++) {
            const idx = isA ? this._wallGroupAIndex % total : this._wallGroupBIndex % total; // 推进索引
        ...
        setRandomScaleAndHealth(enemyNode) {
          ...
          const minScale = 0.9;
          const maxScale = 1;
          const randomScale = minScale + Math.random() * (maxScale - minScale); // 设置节点缩放

          enemyNode.setScale(randomScale, randomScale, randomScale); // 根据缩放值计算对应生命值：160到240

          const minHealth = 140;
          const maxHealth = 260; // 缩放值与生命值成正比

          const scaleRatio = (randomScale - minScale) / (maxScale - minScale); // 0到1的比例

          const calculatedHealth = Math.round(minHealth + scaleRatio * (maxHealth - minHealth)); // 设置生命值

          const healthComponent = enemyNode.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
            error: Error()
        ...
        isPositionSafeFromHome(position) {
          ...
          const hasHit = PhysicsSystem.instance.raycast(ray, (_crd && PHY_GROUP === void 0 ? (_reportPossibleCrUseOfPHY_GROUP({
            error: Error()
        ...
        */
        ...
        getRangeEnemies(position, range) {
          const rangeEnemies = []; // 使用平方距离来优化性能，避免开平方根运算
