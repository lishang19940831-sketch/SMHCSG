      ...
      _export("Wall", Wall = (_dec = ccclass('Wall'), _dec2 = property({
        ...
        onLoad() {
          ...
          this.wallParent.forEach(wallSegment => {
            ...
            const health = wallSegment.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
              error: Error()
        ...
        _cacheInitColors(node) {
          ...
          if (meshRenderer) {
            ...
            for (let i = 0; i < count; i++) {
              const mat = meshRenderer.getMaterialInstance(i); // mainColor 可能不存在（材质未暴露该属性），默认存 WHITE 作为兜底

              const raw = mat == null ? void 0 : mat.getProperty('mainColor');
        ...
        restoreWallColor(node) {
          ...
          if (meshRenderer) {
            ...
            for (let i = 0; i < count; i++) {
              ...
              const initColor = (_colors$i = colors == null ? void 0 : colors[i]) != null ? _colors$i : Color.WHITE;
