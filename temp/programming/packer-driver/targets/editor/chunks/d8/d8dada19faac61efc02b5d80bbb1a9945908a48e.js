    ...
    execute: function () {
      ...
      _export("Hero", Hero = (_dec = ccclass('Hero'), _dec2 = property({
        ...
        onPickupItemFull(itemType) {
          ...
          const currentTime = Date.now(); // 限制提示显示频率（1秒内不重复显示）

          if (currentTime - this.lastTipTime < 1000) {
            return;
          }

          const tipLabel = (_manager$pool$getNode = manager.pool.getNode((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
            error: Error()
        ...
        updateAutoAttack(dt) {
          const currentState = this.stateComponent.getCurrentState(); // 死亡或技能状态不执行自动攻击
        ...
        checkAndAutoAttack() {
          ...
          let enemies = manager.enemy.getRangeEnemies(heroPosition, this.attackComponent.attackRangeValue); // 安全区内且门未打开时，不可攻击敌人
        ...
        onPerformAttack(damageData) {
          const now = Date.now(); // 防抖：100ms 内不重复触发
          ...
          for (const target of primaryTargets) {
            const cornHealth = target.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
              error: Error()
        ...
        getAttackTargetList() {
          ...
          const heroPosition = this.node.getWorldPosition();
          let enemies = manager.enemy.getRangeEnemies(heroPosition, attackRange); // 安全区内且门未打开时，不可攻击敌人
        ...
        setMovementEnabled(enabled) {
          const movement = this.node.getComponent(_crd && MovementComponent === void 0 ? (_reportPossibleCrUseOfMovementComponent({
            error: Error()
          }), MovementComponent) : MovementComponent);
          if (!movement) return;
