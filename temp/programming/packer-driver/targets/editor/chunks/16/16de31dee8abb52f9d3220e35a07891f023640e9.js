      ...
      _export("TrainTrack", TrainTrack = (_dec = ccclass('TrainTrack'), _dec2 = property({
        ...
        expandToPhase(phase, onComplete) {
          ...
          if (phase === TrackPhase.Phase2 && this.trackNodes.length >= 3 && this.expandDuration > 0) {
            ...
            if (oldA && oldB && newRail) {
              ...
              const origB = oldB.scale.clone(); // 新轨道从0逐段出现：按子节点顺序依次放大
        ...
        getProgressByPosition(worldPos) {
          ...
          for (let i = 0; i < n; i++) {
            ...
            const d = Math.sqrt((worldPos.x - closest.x) ** 2 + (worldPos.z - closest.z) ** 2);
        ...
        _buildPath(waypoints) {
          ...
          for (const wp of waypoints) {
            ...
            if (b1 && b1.isValid && b2 && b2.isValid && b3 && b3.isValid) {
              ...
              const p1 = new Vec3(2 * pm.x - 0.5 * p0.x - 0.5 * p2.x, 2 * pm.y - 0.5 * p0.y - 0.5 * p2.y, 2 * pm.z - 0.5 * p0.z - 0.5 * p2.z); // 等弧长采样：先高密度采样建立 t→弧长 映射表
              ...
              const totalArc = arcLens[arcLens.length - 1]; // 按等弧长间隔取点（含起点不含终点，终点由 b_3 单独加入）

              for (let i = 0; i < this.bezierSamples; i++) {
                const targetLen = i / this.bezierSamples * totalArc; // 二分查找对应的 rawPts 索引
                ...
                const frac = arcLens[hi] > arcLens[lo] ? (targetLen - arcLens[lo]) / (arcLens[hi] - arcLens[lo]) : 0;
          ...
          const n = this._points.length;
          let accumulated = 0; // 闭合路径：最后一段从 points[n-1] 回到 points[0]
        ...
        _calcStationProgress() {
          ...
          for (let i = 0; i < n; i++) {
            const a = this._points[i];
            const b = this._points[(i + 1) % n]; // 求 stationPos 在线段 a→b 上的最近点
            ...
            t = Math.max(0, Math.min(1, t));
            const closest = new Vec3(a.x + ab.x * t, 0, a.z + ab.z * t);
            const d = Math.sqrt((stationPos.x - closest.x) ** 2 + (stationPos.z - closest.z) ** 2);
