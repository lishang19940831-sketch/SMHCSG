    ...
    execute: function () {
      ...
      _export("Enemy", Enemy = (_dec = ccclass('Enemy'), _dec2 = property({
        ...
        getDirectionToTarget() {
          ...
          const enemyPos = this.node.getWorldPosition();
          const targetPos = this.targetNode.getWorldPosition(); // 计算方向向量
        ...
        checkIsNearTarget(offset) {
          ...
          const enemyPos = this.node.getWorldPosition();
          const targetPos = this.targetNode.getWorldPosition(); // 只计算 XZ 平面距离，忽略 Y 轴高度差，避免因地形高度差导致永远不触发攻击
        ...
        getDistanceToTarget() {
          ...
          const enemyPos = this.node.getWorldPosition();
          const targetPos = this.targetNode.getWorldPosition(); // 只计算 XZ 平面距离，忽略 Y 轴高度差
        ...
        updateHeroTracking() {
          ...
          const distanceToHero = Vec3.distance(enemyPos, heroPos); // 英雄不在安全区且在检测范围内
        ...
        checkReachedOriginalPosition() {
          ...
          const distance = Vec3.distance(currentPos, this.originalPosition); // 距离小于0.5认为已到达
          ...
          switch (this.aiState) {
            ...
            case EnemyAIState.Attack:
              ...
              if (this.attackComponent.canAttack()) {
                const target = this.GetAttackTarget(); //console.log(`[Enemy攻击] 尝试发起攻击`, {
                //     '敌人': this.node.name,
        ...
        onPerformAttack(damageData) {
          ...
          if (target) {
            ...
            if (healthComponent) {
              ...
              const beforeHpPercent = healthComponent.healthPercentage; //console.log(`[Enemy攻击] 攻击目标: ${target.name}`, {
              ...
              const actualDamage = beforeHp - afterHp; //console.log(`[Enemy攻击] 伤害结果`, {
        ...
        playSpawnIntro(wpos) {
          ...
          const introDur = Math.max(this.spawnIntroDuration, clipDur || 0); // 钻出动画（若配置了对应剪辑名）
