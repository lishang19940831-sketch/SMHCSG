    ...
    execute: function () {
      ...
      _export("ItemContainer", ItemContainer = (_dec = ccclass('ItemContainer'), _dec2 = property({
        ...
        updateCachedEmptyPosition() {
          ...
          const items = this.items[this.currentLayer]; // 计算物品在当前层的位置

          const index = items.length;
          const row = Math.floor(index / this.columns);
          const col = index % this.columns; // 计算45度视角下的目标位置

          const baseX = (col - row) * this.spacing * this.scaleX;
          const baseY = (col + row) * this.spacing * 0.5; // 如果当前层超过最大层数，则使用最大层数的高度
        ...
        addItem(item) {
          // 获取当前空位置
          const targetPos = this.currEmptyPosition; // 确保当前层的数组已初始化
          ...
          const items = this.items[this.currentLayer]; // 将物品添加到容器节点下
        ...
        removeItem(item) {
          ...
          if (itemInfo) {
            ...
            const items = this.items[layer]; // 从数组中移除
        ...
        rearrangeItems(layer, startIndex) {
          ...
          for (let i = startIndex; i < items.length; i++) {
            ...
            const baseX = (col - row) * this.spacing * this.scaleX;
            const baseY = (col + row) * this.spacing * 0.5; // 复用Vec3对象
