    ...
    execute: function () {
      ...
      _export("DropManager", DropManager = (_dec = ccclass('DropManager'), _dec(_class = class DropManager extends Component {
        ...
        cleanupExcessItems(objectType, keepCount) {
          ...
          const targetKeepCount = keepCount != null ? keepCount : limit;
          const itemsOfType = this.itemList.filter(item => item.objectType === objectType);

          if (itemsOfType.length > targetKeepCount) {
            const excessCount = itemsOfType.length - targetKeepCount; //console.log(`清理多余的 ${objectType} 物品，数量: ${excessCount}`);
        ...
        getRangeItems(worldPosition, range) {
          ...
          for (const item of this.itemList) {
            const itemPos = item.node.getWorldPosition(); // 计算距离的平方

            const distanceSquared = Vec3.squaredDistance(worldPosition, itemPos); // 如果距离平方小于等于范围平方，则在范围内
        ...
        spawnItem(objectType, basePosition, isAddDrop = true) {
          // 检查是否达到掉落上限
          ...
          const item = manager.pool.getNode(objectType); // 设置物品初始位置（在生成点上方一点）
        ...
        spawnParabolicItem(objectType, basePosition, dropRadius = 3) {
          // 检查是否达到掉落上限
          if (this.isDropLimitReached(objectType)) {
          ...
          do {
            ...
            const randomOffset = new Vec3((Math.random() - 0.5) * 2 * dropRadius, 0, // 保持在同一高度
            (Math.random() - 0.5) * 2 * dropRadius);
            const targetX = basePosition.x + randomOffset.x;
            const targetZ = basePosition.z + randomOffset.z; // 使用GameManager的地面高度计算方法
        ...
        createParabolicMotion(item, startPos, endPos, callback) {
          ...
          const startRotationY = item.eulerAngles.y; // 随机生成飞行过程中的旋转速度（每秒转动的角度）
          ...
          tween(item).to(duration, {}, {
            onUpdate: (target, ratio) => {
              ...
              const parabolicHeight = 4 * 3 * heightVariation * ratio * (1 - ratio); // 在起点和终点之间进行线性插值，再加上抛物线高度
