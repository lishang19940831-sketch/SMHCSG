  ...
  return {
    ...
    execute: function () {
      ...
      _export("WallManager", WallManager = (_dec = ccclass('WallManager'), _dec2 = property({
        ...
        getAttackTarget() {
          // 检查外围墙是否解锁
          var outsideWallUnlockItem = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
            error: Error()
          }), BuildingType) : BuildingType).Wall1);
          var isOutsideWallUnlocked = outsideWallUnlockItem && outsideWallUnlockItem.UnlockState === (_crd && BuildUnlockState === void 0 ? (_reportPossibleCrUseOfBuildUnlockState({
            error: Error()
        ...
        checkIsInRange(target, offset) {
          ...
          try {
            ...
            this.rangeNode.forEach(node => {
              ...
              var rangePos = node.getWorldPosition();
              var distance = Vec3.distance(targetPos, rangePos);
              var rangeRadius = Math.max(node.scale.x, node.scale.z) / 2; // 使用X和Z轴的最大值
              ...
              var extraRange = isOutsideWallUnlocked ? 5 : 0;
              var result = distance <= offset + rangeRadius + extraRange; // // 调试信息
              // if (result) {
