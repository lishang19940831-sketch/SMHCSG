    ...
    execute: function () {
      ...
      _export("ProductionBuilding", ProductionBuilding = (_dec = ccclass('ProductionBuilding'), _dec2 = property({
        ...
        update(dt) {
          ...
          var hasWorker = this.pickupComponents.size > 0;
          var hasEnoughMaterial = this.itemLayout.getItemCount() >= this.consumePerProduction;
        ...
        _tryReceiveMaterial(pickupComponent, duration) {
          var itemCount = pickupComponent.getItemCount(this.rawMaterialType);
          if (itemCount <= 0) return;
          var extraCount = Math.floor(duration / 0.1);
          var consumeCount = Math.min(1 + extraCount, 20);
          consumeCount = Math.min(consumeCount, itemCount);
          var consumedList = pickupComponent.consumeItem(this.rawMaterialType, consumeCount);

          for (var item of consumedList) {
        ...
        _flyMaterialToLayout(item) {
          ...
          var layoutPos = this._takeRefillPosition();
          ...
          if (!layoutPos) {
            // 原料槽已满：飞到最后一个有效位置后回收
            var lastPos = this.itemLayout.getLastValidPosition();
          ...
          var start = item.getWorldPosition().clone();
          start.y += 1;
          var progressObj = {
            t: 0
          };
          ...
          tween(progressObj).to(0.4, {
            ...
            onUpdate: () => {
              ...
              var liveTarget = layoutPos && !isFull ? this.itemLayout.getItemPosition(layoutPos) : targetPos;
              var x = start.x + (liveTarget.x - start.x) * progressObj.t;
              var z = start.z + (liveTarget.z - start.z) * progressObj.t;
              var y = start.y + (liveTarget.y - start.y) * progressObj.t + 4 * arcH * progressObj.t * (1 - progressObj.t);
            ...
            if (isFull) {
              ...
              var success = this.itemLayout.addItemToReserve(item, layoutPos);
        ...
        _produce() {
          ...
          var rawCount = this.itemLayout.getItemCount();
          if (rawCount < this.consumePerProduction) return; // 消耗原料，同时记录每个被消耗原料的世界坐标

          var consumedPositions = [];

          for (var i = 0; i < this.consumePerProduction; i++) {
            ...
            var lpos = this._findItemPosition(node);
            ...
            if (lpos) {
              ...
              var foundReserve = false;

              for (var layer = this.itemLayout.maxLayerLimit - 1; layer >= 0; layer--) {
                for (var row = this.itemLayout.rows - 1; row >= 0; row--) {
                  for (var column = this.itemLayout.columns - 1; column >= 0; column--) {
                    var rpos = {
                      row,
                      column,
                      layer
                    };
                    var item = this.itemLayout.getItem(rpos);
          ...
          var _loop = function _loop() {
            var posIndex = Math.min(_i, consumedPositions.length - 1);
            var fromPos = consumedPositions[posIndex].clone();

            _this.scheduleOnce(() => {
              var _this$outputContainer;

              (_this$outputContainer = _this.outputContainer) == null || _this$outputContainer.receive(fromPos);
            }, _i * 0.05);
          };

          for (var _i = 0; _i < this.outputPerProduction; _i++) {
            _loop();
          }
        } // ──────────────────────────────────────────────
        // 公开接口
        ...
        receiveRawMaterial(fromPosition, type) {
          ...
          var item = manager.pool.getNode(type);
          ...
          tween(progressObj2).to(0.4, {
            ...
            onUpdate: () => {
              if (!item.isValid) return;
              var liveTarget = layoutPos && !isFull ? this.itemLayout.getItemPosition(layoutPos) : targetPos;
              var x = startPos.x + (liveTarget.x - startPos.x) * progressObj2.t;
              var z = startPos.z + (liveTarget.z - startPos.z) * progressObj2.t;
              var y = startPos.y + (liveTarget.y - startPos.y) * progressObj2.t + 4 * arcH2 * progressObj2.t * (1 - progressObj2.t);
              item.setWorldPosition(x, y, z);
            }
          }).call(() => {
            if (!item.isValid) {
        ...
        _findItemPosition(node) {
          for (var layer = 0; layer < this.itemLayout.maxLayerLimit; layer++) {
            for (var row = 0; row < this.itemLayout.rows; row++) {
              for (var column = 0; column < this.itemLayout.columns; column++) {
                var lpos = {
                  row,
                  column,
                  layer
                };
                var it = this.itemLayout.getItem(lpos); // 放宽条件：只要节点匹配，不管是不是 isAdded，都认为是它的位置（兼容飞行中的节点）
        ...
        _takeRefillPosition() {
          for (var i = 0; i < this._refillPositions.length; i++) {
            var candidate = this._refillPositions[i];
            var it = this.itemLayout.getItem(candidate);
        ...
        _pushRefillPosition(lpos) {
          var exists = this._refillPositions.some(p => p.row === lpos.row && p.column === lpos.column && p.layer === lpos.layer);
