    ...
    execute: function () {
      ...
      _export("BulletBase", BulletBase = (_dec = ccclass('BulletBase'), _dec2 = property({
        ...
        _updateRotation() {
          ...
          var axisForward = new Vec3(this._direction.x, this._direction.y, this._direction.z);
          ...
          axisUp.normalize();
          var q = new Quat(); // 模型正面朝 +X：X轴=forward, Y轴=up, Z轴=-right
        ...
        _initParabola() {
          ...
          var height = this._calculateParabolaHeight(this._parabolaCurve.distance); // 设置控制点
        ...
        _calculateTargetPosition() {
          ...
          if (this.targetType === BulletTargetType.TRACKING && (_this$targetNode = this.targetNode) != null && _this$targetNode.isValid) {
            var targetPos = this.targetNode.getWorldPosition(); // 使用地面高度作为抛物线终点，而不是目标节点的高度
        ...
        _calculateParabolaHeight(distance) {
          ...
          var height = this.parabolaBaseHeight + distance * this.parabolaHeightFactor; // 限制最大高度
        ...
        _updateParabola(dt) {
          ...
          var progressDelta = this.speed * dt / this._parabolaCurve.distance;
          this._parabolaCurve.progress = Math.min(this._parabolaCurve.progress + progressDelta, 1.0); // 计算新位置

          var newPos = this._quadraticBezier(this._parabolaCurve.start, this._parabolaCurve.control, this._parabolaCurve.end, this._parabolaCurve.progress); // 更新距离和位置


          var lastPos = this.node.getWorldPosition();
          ...
          if (gameManager) {
            var groundHeight = gameManager.calculateGroundHeight(newPos);
            var groundThreshold = 0.5; // 地面检测容差，高度差小于此值认为碰撞
          ...
          if (this._parabolaCurve.progress >= 1.0) {
            ...
            var _gameManager = manager.game;

            if (_gameManager) {
              var _groundHeight = _gameManager.calculateGroundHeight(currentPos); // 抛物线结束时，无论高度如何都触发地面爆炸


              this.onGroundHit(currentPos, _groundHeight);
              return;
            } // 如果没有游戏管理器，直接回收
        ...
        _updateParabolaDirection() {
          ...
          var nextProgress = Math.min(this._parabolaCurve.progress + 0.02, 1.0);

          var nextPos = this._quadraticBezier(this._parabolaCurve.start, this._parabolaCurve.control, this._parabolaCurve.end, nextProgress);
        ...
        _updateTrackingTarget() {
          ...
          var health = this.targetNode.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
            error: Error()
          ...
          var height = this._calculateParabolaHeight(this._parabolaCurve.distance);
        ...
        _onCollisionEnter(event) {
          ...
          var targetNode = ((_otherCollider = event.otherCollider) == null ? void 0 : _otherCollider.node) || ((_otherCollider2 = event.otherCollider) == null ? void 0 : _otherCollider2.node);
        ...
        _checkDistanceHit() {
          var enemyManager = manager.enemy;
          if (!enemyManager) return;
          var bulletPos = this.node.getWorldPosition();
          var enemies = enemyManager.getRangeEnemies(bulletPos, this.hitDistance); // 查找未命中的敌人
        ...
        _checkGroundHeight() {
          ...
          var currentPos = this.node.getWorldPosition();
          var groundHeight = gameManager.calculateGroundHeight(currentPos);
          var groundThreshold = 0.5; // 地面检测容差
        ...
        _updateTracking(dt) {
          ...
          var rotateWeight = Math.min(this.trackingSpeed * dt, 1.0);
