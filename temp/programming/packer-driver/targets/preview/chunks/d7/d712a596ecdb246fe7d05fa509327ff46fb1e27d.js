      ...
      _export("EffectManager", EffectManager = (_dec6 = ccclass('EffectManager'), _dec7 = property({
        ...
        static get instance() {
          if (!this._instance) {
            ...
            // 动态创建节点并添加组件
            var node = new Node('EffectManager');
        ...
        initPools() {
          for (var item of this.effectItems) {
        ...
        playEffect(type, position, rotation, parent, scale) {
          var item = this.getEffectByType(type);
          ...
          if (item.playType === EffectPlayType.Animation) {
            var animss = effect.getComponentsInChildren(Animation);
            var completedCount = 0;
            var totalAnims = animss.length;
        ...
        recycleEffect(effect) {
          ...
          var type = this._activeEffects.get(effect);
        ...
        recycleAllEffects() {
          var effects = Array.from(this._activeEffects.keys());
        ...
        flyNodeInParabola(params) {
          ...
          var isNodeTarget = target instanceof Node;
          var startPos = node.getWorldPosition();
          var targetPos = isNodeTarget ? target.getWorldPosition() : target; // 获取起始缩放和旋转

          var startScale = node.getWorldScale();
          var startRotation = node.getWorldRotation(); // 创建一个临时对象来驱动动画进度
          ...
          tween(progressObj).to(duration, {
            ...
            onUpdate: () => {
              var ratio = progressObj.progress;
              ...
              var currentX = startPos.x + (targetPos.x - startPos.x) * ratio;
              var currentZ = startPos.z + (targetPos.z - startPos.z) * ratio; // 计算Y位置（抛物线轨迹）
              // 使用公式: y = 4 * h * t * (1 - t)，其中t是[0,1]之间的进度值
              // 这个公式会创建一个在中点达到最大高度的抛物线

              var heightOffset = 4 * height * ratio * (1 - ratio);
              var currentY = startPos.y + (targetPos.y - startPos.y) * ratio + heightOffset;
              ...
              if (targetWorldScale || isNodeTarget) {
                var finalTargetScale = targetWorldScale;
                ...
                if (finalTargetScale) {
                  var currentScale = new Vec3(startScale.x + (finalTargetScale.x - startScale.x) * ratio, startScale.y + (finalTargetScale.y - startScale.y) * ratio, startScale.z + (finalTargetScale.z - startScale.z) * ratio);
              ...
              if (targetWorldRotation || isNodeTarget) {
                var finalTargetRotation = targetWorldRotation;
