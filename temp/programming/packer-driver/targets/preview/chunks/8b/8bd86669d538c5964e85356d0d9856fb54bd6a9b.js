    ...
    execute: function () {
      ...
      _export("FlightManager", FlightManager = (_dec = ccclass('FlightManager'), _dec(_class = (_class2 = class FlightManager extends Component {
        ...
        createFlight(flyNode, startPos, endNode, options) {
          ...
          // 合并默认选项
          var finalOptions = _extends({}, FlightManager.DEFAULT_OPTIONS, options); // 生成唯一ID


          var itemId = "flight_" + ++this.itemCounter; // 设置节点父级 - 修改为添加到最底层
          ...
          var distance = this.calculateApproximateDistance(flightItem.startPos, flightItem.controlPoint, flightItem.endPos);
        ...
        startFlightTween(item, options) {
          ...
          var progressTween = tween(item).to(item.duration, {
            progress: 1
          }, {
            ...
            onUpdate: () => {
              ...
              var position = this.calculateBezierPosition(item.startPos, item.controlPoint, item.endPos, item.progress); // 更新节点位置
        ...
        updateNodeRotation(item, currentPos) {
          // 计算下一个位置点
          var nextProgress = Math.min(item.progress + 0.05, 1);
          var nextPos = this.calculateBezierPosition(item.startPos, item.controlPoint, item.endPos, nextProgress); // 计算方向向量
        ...
        calculateApproximateDistance(start, control, end) {
          ...
          var segments = 10;
          var distance = 0;
          var prevPoint = start.clone();

          for (var i = 1; i <= segments; i++) {
            var _t = i / segments;

            var point = this.calculateBezierPosition(start, control, end, _t);
        ...
        calculateBezierPosition(start, control, end, t) {
          var x = this.calculateBezierPoint(start.x, control.x, end.x, t);
          var y = this.calculateBezierPoint(start.y, control.y, end.y, t);
          var z = this.calculateBezierPoint(start.z, control.z, end.z, t);
        ...
        removeFlight(itemId) {
          var item = this.flightItems.get(itemId);
        ...
        updateFlightTargetPosition(itemId, newEndNodeOffset, options) {
          ...
          var distance = this.calculateApproximateDistance(item.startPos, item.controlPoint, item.endPos);
          var speed = options.speed || FlightManager.DEFAULT_OPTIONS.speed || 1000;
          item.duration = distance / speed; // 重置进度

          item.progress = 0; // 重新开始飞行动画

          this.startFlightTween(item, _extends({}, FlightManager.DEFAULT_OPTIONS, options, {
