    ...
    execute: function () {
      ...
      _export("MainCamera", MainCamera = (_dec = ccclass('MainCamera'), _dec2 = property({
        ...
        isSourceInCameraView(source) {
          ...
          var cameraPos = this.camera.node.getWorldPosition();
          var sourcePos = source.getWorldPosition(); // 计算距离

          var distance = Vec3.distance(cameraPos, sourcePos); // 设置可见距离范围（可以根据需要调整）

          var maxDistance = 1000; // 最大震动距离
          ...
          var screenPos = this.camera.worldToScreen(sourcePos); // 获取屏幕尺寸 - 使用更简单的固定值或相机的渲染范围

          var screenWidth = 720; // 可以根据项目设置调整

          var screenHeight = 1280; // 可以根据项目设置调整
          // 判断是否在屏幕范围内（包含一定的边界扩展）

          var margin = 200; // 边界扩展，允许稍微超出屏幕的物体也能触发震动
        ...
        lateUpdate(dt) {
          if (this.targetPosition && this.isFollowing) {
            var currentPos = this.cameraParent.getWorldPosition();
            var smoothedPos = new Vec3(); // 使用插值实现平滑跟随

            var t = Math.min(dt / this.smoothTime, 1.0);
