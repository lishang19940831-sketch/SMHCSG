      ...
      _export("ItemLayout", ItemLayout = (_dec = ccclass('ItemLayout'), _dec2 = property({
        ...
        addItem(node, lpos) {
          ...
          if (lpos) {
            ...
            var _key = this.getKey(lpos);

            var existingItem = this.items.get(_key);
          ...
          var worldPos = this.getItemPosition(position);
        ...
        reserveItem(lpos) {
          ...
          var key = this.getKey(lpos);
          var data = this.items.get(key);
        ...
        addItemToReserve(node, lpos) {
          ...
          if (data) {
            if (data.isFull == true && data.isAdded == false) {
              ...
              var result = this.addItem(node); // addItem 方法内部已经会触发事件，所以这里不需要重复触发
        ...
        removeItem(lpos) {
          var key = this.getKey(lpos); // const item = this.items.get(key);
          // if (item && item.node) {
          //     item.node.removeFromParent();
          // }

          var result = this.items.delete(key);
        ...
        removeItemByNode(node) {
          for (var [key, item] of this.items) {
            if (item.node === node) {
        ...
        getCurrEmptyPosition() {
          ...
          for (var layer = 0; layer < this.maxLayer; layer++) {
        ...
        getOuterItems(num) {
          ...
          for (var layer = this.maxLayer - 1; layer >= 0; layer--) {
            for (var row = this.rows - 1; row >= 0; row--) {
              for (var column = this.columns - 1; column >= 0; column--) {
                var key = this.getKey({
                  row,
                  column,
                  layer
        ...
        clearAllItems() {
          var hadItems = this.getItemCount() > 0;
        ...
        getItemPosition(position) {
          // 获取局部坐标偏移
          var localOffset = this._caculateItemPosition(position); // 创建临时结果向量


          var result = new Vec3(); // 使用节点的世界矩阵将局部坐标转换为世界坐标
