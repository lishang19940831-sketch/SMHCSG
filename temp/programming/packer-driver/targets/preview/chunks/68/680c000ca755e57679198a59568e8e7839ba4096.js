      ...
      _export("ConveyorBelt", ConveyorBelt = (_dec = ccclass('ConveyorBelt'), _dec2 = property({
        ...
        update(dt) {
          ...
          if (this.node.active && this.conveyorEnabled && this.conveyorMaterial) {
            ...
            var v = new Vec4(1, 1, this._uvZ, 0);
        ...
        _trySpawnOne() {
          ...
          var sink = this._getSink();

          if (!source || !sink) return;

          var wp = this._buildWaypoints();

          if (wp.length < 2) return;
          var item = source.takeForConveyor == null ? void 0 : source.takeForConveyor();
          if (!item) return;
          this._inFlight++;
          item.setParent(manager.effect.node);
          var start = wp[0].clone();
          ...
          for (var i = 1; i < wp.length; i++) {
            ...
            var dur = this.moveSpeed > 0 ? dist / this.moveSpeed : 0.01;
            var target = new Vec3(to.x, to.y + this.visualYOffset, to.z);
            seq = seq.to(dur, {
          ...
          seq.call(() => {
            var pos = item.getWorldPosition().clone();
            manager.pool.putNode(item);

            var accepted = this._deliverToSink(sink, pos, () => {
        ...
        _buildWaypoints() {
          ...
          var pts = [];
          var aNode = (_this$pointA = this.pointA) != null ? _this$pointA : this.sourceNode;
          if (aNode && aNode.isValid) pts.push(aNode.getWorldPosition().clone());

          for (var n of this.midPoints) {
        ...
        _getSource() {
          ...
          var wc = this.sourceNode.getComponent(_crd && WheatContainer === void 0 ? (_reportPossibleCrUseOfWheatContainer({
            error: Error()
        ...
        _getSink() {
          ...
          var pb = this.sinkNode.getComponent(_crd && ProductionBuilding === void 0 ? (_reportPossibleCrUseOfProductionBuilding({
            error: Error()
          }), ProductionBuilding) : ProductionBuilding);
          if (pb) return pb;
          var wc = this.sinkNode.getComponent(_crd && WheatContainer === void 0 ? (_reportPossibleCrUseOfWheatContainer({
            error: Error()
          }), WheatContainer) : WheatContainer);
          if (wc) return wc;
          var fb = this.sinkNode.getComponent(_crd && FlatbreadContainer === void 0 ? (_reportPossibleCrUseOfFlatbreadContainer({
            error: Error()
        ...
        _deliverToSink(sink, from, done) {
          ...
          if (pb && pb.receiveRawMaterial) {
            var ok = pb.receiveRawMaterial(from, this.itemType);
            done();
            return ok;
          }
