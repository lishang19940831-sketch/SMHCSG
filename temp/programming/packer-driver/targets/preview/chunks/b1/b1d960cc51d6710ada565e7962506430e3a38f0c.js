    ...
    execute: function () {
      ...
      _export("Shell", Shell = (_dec = ccclass('Shell'), _dec(_class = class Shell extends (_crd && BulletBase === void 0 ? (_reportPossibleCrUseOfBulletBase({
        ...
        onHit(targetNode) {
          ...
          if (targetNode) {
            ...
            if (healthComp) {
              ...
              var moveComp = targetNode.getComponent(_crd && MovementComponent === void 0 ? (_reportPossibleCrUseOfMovementComponent({
                error: Error()
              ...
              var aoeNodes = manager.enemy.getRangeEnemies(hitPos, 4);

              for (var item of aoeNodes) {
                var _healthComp = item.node.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
                  error: Error()
                }), HealthComponent) : HealthComponent);

                if (_healthComp) {
                  _healthComp.takeDamage({
                    damage: this.damage,
                    damageSource: this.node
                  });
                }

                var _moveComp = item.node.getComponent(_crd && MovementComponent === void 0 ? (_reportPossibleCrUseOfMovementComponent({
                  error: Error()
                }), MovementComponent) : MovementComponent);

                if (_moveComp) {
                  _moveComp.knockup(4, Vec3.subtract(new Vec3(), item.node.getWorldPosition(), this.node.getWorldPosition()));
                }
              } // 播放受击特效
        ...
        onGroundHit(hitPosition, groundHeight) {
          // 在落地点播放爆炸特效
          var explosionPos = new Vec3(hitPosition.x, groundHeight, hitPosition.z);
          manager.effect.playEffect((_crd && EffectType === void 0 ? (_reportPossibleCrUseOfEffectType({
            error: Error()
          }), EffectType) : EffectType).Explosion, explosionPos);
          app.audio.playEffect('resources/audio/爆炸'); // AOE范围伤害

          var aoeNodes = manager.enemy.getRangeEnemies(explosionPos, 4);

          for (var item of aoeNodes) {
            var healthComp = item.node.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
              error: Error()
            }), HealthComponent) : HealthComponent);
            ...
            var moveComp = item.node.getComponent(_crd && MovementComponent === void 0 ? (_reportPossibleCrUseOfMovementComponent({
              error: Error()
            }), MovementComponent) : MovementComponent);
