    ...
    execute: function () {
      ...
      _export("TrainCar", TrainCar = (_dec = ccclass('TrainCar'), _dec2 = property({
        ...
        onLoad() {
          ...
          if (this.itemLayout) {
            var layoutCapacity = this.itemLayout.maxLayerLimit * this.itemLayout.rows * this.itemLayout.columns;
        ...
        updateOnTrack(headProgress, updateFacing) {
          ...
          var totalLength = this._track.getTotalLength();

          if (totalLength <= 0) return; // 本节车厢的归一化进度（落后车头一段距离）

          var delta = this._distanceBehindHead / totalLength;
          var myProgress = ((headProgress - delta) % 1 + 1) % 1; // 设置世界坐标

          var pos = this._track.getPositionAt(myProgress);

          this.node.setWorldPosition(pos); // 设置朝向（仅行驶时更新）

          if (updateFacing) {
            var dir = this._track.getDirectionAt(myProgress);
        ...
        addResource(type, amount) {
          ...
          var canAdd = Math.min(amount, this.capacity - this._currentLoad);
        ...
        addItemNode(node, lpos) {
          ...
          if (lpos) {
            ...
            var placed = this.itemLayout.addItem(node, lpos);
          ...
          var reserved = this._reservedQueue.shift();

          if (reserved) {
            this.itemLayout.removeItem(reserved);

            var _placed = this.itemLayout.addItem(node, reserved);
          ...
          var result = this.itemLayout.addItem(node);
        ...
        getTargetWorldPos() {
          ...
          if (this.itemLayout.isLayerLimitExceeded()) {
            var last = this.itemLayout.getLastValidPosition();
            return this.itemLayout.getItemPosition(last);
          }

          var pos = this.itemLayout.getCurrEmptyPosition();
          if (!pos) return null;
