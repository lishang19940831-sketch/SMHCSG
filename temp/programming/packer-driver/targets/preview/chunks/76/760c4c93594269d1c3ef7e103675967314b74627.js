    ...
    execute: function () {
      ...
      GuideConfig = {
        ...
        [GuideStep.PickUpCoin]: {
          ...
          arrowTopHeight: 1.5,
          condition: () => {
            var coinContainer = manager.game.flatbreadShop.CoinContainer;
        ...
        [GuideStep.UnlockSeller1]: {
          ...
          arrowTopHeight: 0.35,
          condition: () => {
            var unlockItem = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
              error: Error()
            }), BuildingType) : BuildingType).Salesperson1);
        ...
        [GuideStep.UpgradeTrainToLv2]: {
          ...
          condition: () => {
            //检查当前火车等级
            var trainLv = manager.game.trainManager.getLevel();
      ...
      _export("GuideManager", GuideManager = (_dec = ccclass('GuideManager'), _dec2 = property({
        ...
        refreshArrowTarget() {
          ...
          try {
            // 重新获取endNode
            var newEndNode = typeof this._currentConfig.endNode === 'function' ? this._currentConfig.endNode() : this._currentConfig.endNode; // 如果endNode发生了变化，更新引用
        ...
        updateArrowPosition() {
          ...
          var startWPosY = manager.game.calculateGroundHeight(this._startNode.getWorldPosition());

          var startWPos = this._startNode.getWorldPosition();

          startWPos.y = startWPosY + 0.05;
          var endWPosY = manager.game.calculateGroundHeight(this._endNode.getWorldPosition());

          var endWPos = this._endNode.getWorldPosition();
        ...
        updateArrow(startWPos, endWPos) {
          ...
          var distance = Vec3.distance(startWPos, endWPos);
          this.arrow.active = true;
          this.arrow1.active = false;
          this.arrowTop.active = true;
          var topArrowPos = endWPos.clone();
          var arrowHeight = ((_this$_currentConfig = this._currentConfig) == null ? void 0 : _this$_currentConfig.arrowTopHeight) || 2;
          ...
          if (this.navLineRoot && this.navLineRoot.isValid) {
            var navComp = this.navLineRoot.getComponent('NavLineComp');
        ...
        onUnlockItem(buildingType) {
          ...
          var expectedStep = buildingToGuideMap[buildingType];
        ...
        checkCurrentGuideComplete() {
          ...
          switch (this._currentStep) {
            ...
            case GuideStep.GoToProductionBuilding:
              ...
              var currentCornCount = hero.GetItemCount((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
                error: Error()
            ...
            case GuideStep.GoToPieShop:
              // 生产建筑交付引导：麦子数量减少（从有到无）
              var currentFlatbreadCount = hero.GetItemCount((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
                error: Error()
            ...
            case GuideStep.UnlockArrowTower1:
              //检查 箭塔1是否已解锁
              var ufoUnlockItem1 = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
                error: Error()
              }), BuildingType) : BuildingType).ArrowTower);
            ...
            case GuideStep.UnlockArrowTower2:
              //检查 箭塔2是否已解锁
              var ufoUnlockItem2 = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
                error: Error()
              }), BuildingType) : BuildingType).ArrowTower1);
            ...
            case GuideStep.UnlockArrowTower3:
              //检查 箭塔3是否已解锁
              var ufoUnlockItem3 = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
                error: Error()
              }), BuildingType) : BuildingType).ArrowTower2);
            ...
            case GuideStep.UnlockArrowTower4:
              //检查 箭塔4是否已解锁
              var ufoUnlockItem4 = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
                error: Error()
              }), BuildingType) : BuildingType).ArrowTower3);
              return (ufoUnlockItem4 == null ? void 0 : ufoUnlockItem4.unlockState) === "Unlocked";

            case GuideStep.ExpandField:
              var ufoUnlockItem = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
                error: Error()
              }), BuildingType) : BuildingType).EndGame);
        ...
        tryTriggerGuide() {
          try {
            ...
            var guideSteps = [GuideStep.BoardTrain, GuideStep.GoToWarehouse, GuideStep.GoToProductionBuilding, GuideStep.PickUpPie, GuideStep.GoToPieShop, GuideStep.PickUpCoin, GuideStep.UnlockSeller1, // GuideStep.UnlockSeller2,
        ...
        canTriggerGuide(step) {
          ...
          if (config.condition) {
            var conditionResult = config.condition(); // console.log(`[GuideManager] 引导步骤 ${step} 条件检查结果: ${conditionResult}`);
        ...
        startGuide(step) {
          ...
          var endNode = typeof config.endNode === 'function' ? config.endNode() : config.endNode;
        ...
        updateGuideArrow() {
          if (!this._currentConfig) return;
          var startNode = manager.game.hero.node;
          var endNode = typeof this._currentConfig.endNode === 'function' ? this._currentConfig.endNode() : this._currentConfig.endNode;
        ...
        static getCurrTree() {
          try {
            ...
            var heroNode = manager.game.hero.node;
            ...
            var nearestTree = manager.tree.getNearestTree(heroNode.getWorldPosition());
        ...
        static checkGoldEnoughForUnlock(buildingType) {
          ...
          var hero = manager.game.hero;
          var coinCount = hero ? hero.GetItemCount((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
            error: Error()
          }), ObjectType) : ObjectType).DropItemCoin) : 0;
          var unlockItem = ((_manager$game$unlockI = manager.game.unlockItemMap.get(buildingType)) == null ? void 0 : _manager$game$unlockI.getDisplayRemainGold()) || 0;
        ...
        static checkWoodEnoughForUnlock(buildingType) {
          ...
          var hero = manager.game.hero;
          var woodCount = hero ? hero.GetItemCount((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
            error: Error()
          }), ObjectType) : ObjectType).DropItemWood) : 0;
          var unlockItem = ((_manager$game$unlockI2 = manager.game.unlockItemMap.get(buildingType)) == null ? void 0 : _manager$game$unlockI2.getDisplayRemainGold()) || 0;
          return woodCount >= unlockItem;
        }

        static getEndNode(buildingType) {
          var unlockItem = manager.game.unlockItemMap.get(buildingType);
            ...
            if (coinCount > 0 || GuideManager.checkGoldEnoughForUnlock(buildingType)) {
              ...
              if (manager.game.hero.GetItemCount((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
                ...
                if (manager.game.meatShop.CoinContainer.getCount() > 0) {
                  ...
                  var meatCount = hero ? hero.GetItemCount((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
                    error: Error()
                  }), ObjectType) : ObjectType).DropItemMeat) : 0;

                  if (meatCount > 0) {
                    var _unlockItem = manager.game.unlockItemMap.get((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
                      error: Error()
                    }), BuildingType) : BuildingType).ArrowTower);

                    if (_unlockItem.UnlockState == (_crd && BuildUnlockState === void 0 ? (_reportPossibleCrUseOfBuildUnlockState({
                      error: Error()
                    }), BuildUnlockState) : BuildUnlockState).Unlocked) {
                      return _unlockItem.node;
                    } else {
