  ...
  return {
    ...
    execute: function () {
      ...
      _export("TurretTower", TurretTower = (_dec = ccclass('TurretTower'), _dec2 = property({
        ...
        _findNearestEnemy() {
          var towerPos = this.node.getWorldPosition();
        ...
        _canAttack() {
          ...
          var currentTime = Date.now() / 1000; // 如果正在连击中
        ...
        _performAttack() {
          ...
          var currentTime = Date.now() / 1000; // 发射箭矢
        ...
        _fireArrow() {
          ...
          var arrowNode = manager.pool.getNode(this.arrowType); // 设置箭矢位置

          var firePos = this.firePoint.getWorldPosition();
          arrowNode.setWorldPosition(firePos); // 获取箭矢组件

          var arrow = arrowNode.getComponent(_crd && BulletBase === void 0 ? (_reportPossibleCrUseOfBulletBase({
            error: Error()
          ...
          var direction = this._calculateFireDirection();
        ...
        _updateTowerRotation(dt) {
          ...
          var targetPos = this._currentTarget.getWorldPosition(); // 计算方向向量（Y轴指向目标）
          ...
          if (this.yRotationLimit > 0) {
            // 计算相对于初始位置的角度限制
            var relativeTargetAngle = this._targetRotationY;
            var clampedAngle = clamp(relativeTargetAngle, -this.yRotationLimit, this.yRotationLimit);
            this._targetRotationY = clampedAngle;
          } // 检查是否需要旋转（优化性能）


          var angleDiff = Math.abs(this._targetRotationY - this._currentRotationY);
          if (angleDiff < 0.1) return; // 如果角度差异很小，不需要旋转
          // 平滑旋转到目标角度

          if (this.rotationSpeed > 0) {
            var maxRotationThisFrame = this.rotationSpeed * dt; // 选择最短旋转路径
        ...
        _playFireEffect() {
          ...
          // 处理粒子特效
          var particles = this.attEff.getComponentsInChildren(ParticleSystem);
        ...
        onDrawGizmos() {
          ...
          var pos = this.node.getWorldPosition(); // 绘制攻击范围圆圈
          ...
          for (var i = 0; i < segments; i++) {
            ...
            var z2 = pos.z + Math.sin(angle2) * this.attackRange; // 这里应该使用 Cocos Creator 的 Gizmos API，但为简化使用 console.log
