    ...
    execute: function () {
      ...
      _export("Lumberjack", Lumberjack = (_dec = ccclass('Lumberjack'), _dec2 = property({
        ...
        updateAutoAttack(dt) {
          ...
          var currentState = this.stateComponent.getCurrentState();
        ...
        getAvailableTrees(center, range) {
          // 1. 尝试搜索目标列表
          var trees = manager.tree.getRangeTrees(center, range, this.targetTreeListType).filter(tree => tree && tree.node && tree.node.isValid && !tree.isDead); // 2. 如果目标列表为空，尝试搜索备用列表

          if (trees.length === 0) {
            var fallbackListType = this.targetTreeListType === (_crd && TreeListType === void 0 ? (_reportPossibleCrUseOfTreeListType({
              error: Error()
        ...
        checkAndAutoAttack() {
          ...
          var heroPosition = this.node.getWorldPosition(); // 获取范围内的目标列表的树木（支持回退机制）

          var trees = this.getAvailableTrees(heroPosition, attackRange); // 有任何有效目标就攻击
        ...
        onPerformAttack(damageData) {
          ...
          var target = primaryTargets[0]; // 检查目标是否存在（防止多个伐木工同时攻击导致目标消失）
          ...
          var tree = target.getComponent(_crd && Tree === void 0 ? (_reportPossibleCrUseOfTree({
            error: Error()
          }), Tree) : Tree);
        ...
        getAttackTargetList() {
          ...
          var heroPosition = this.node.getWorldPosition(); // 收集目标及其距离信息

          var targetsWithDistance = []; // 获取范围内的目标列表的树木，只选择最近的一棵（支持回退机制）

          var trees = this.getAvailableTrees(heroPosition, attackRange);
          var closestTree = null;

          for (var tree of trees) {
            // 增强检查：确保树对象和节点都有效且未死亡
            if (tree && tree.node && tree.node.isValid && !tree.isDead) {
              var distanceSquared = Vec3.squaredDistance(heroPosition, tree.node.getWorldPosition());
        ...
        GetAttackTarget() {
          var targets = this.getAttackTargetList();
          var target = targets[0]; // 验证目标有效性
        ...
        completeDelivery() {
          ...
          var currentWoodCount = this.pickupComponent.getItemCount((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
            error: Error()
        ...
        getNearestAvailableTree(center) {
          // 1. 尝试搜索目标列表
          var tree = manager.tree.getNearestTree(center, undefined, this.targetTreeListType); // 2. 如果目标列表为空，尝试搜索备用列表
        ...
        returnToChopping() {
          var heroPos = this.node.getWorldPosition(); // const listName = this.targetTreeListType === TreeListType.LIST_1 ? '第一个列表' : '第二个列表';
          // 查找最近的目标列表活树（支持回退机制）

          var nearestTree = this.getNearestAvailableTree(heroPos);

          if (nearestTree) {
            var treePos = nearestTree.getWorldPosition();
            var distance = Vec3.distance(heroPos, treePos); //console.log(`[Lumberjack] 交付完成，返回砍树，距离: ${distance.toFixed(2)}`);
            // 如果已经在攻击范围内，直接开始砍树
            ...
            var targetPos = this.calculateTargetPosition(treePos, heroPos);
        ...
        onTargetReached() {
          ...
          if (this.isDelivering && this.shopPos) {
            ...
            var distance = Vec3.distance(currentPos, this.shopPos); // 如果到达商店附近，触发交付
        ...
        searchForInitialTrees() {
          ...
          var listName = this.targetTreeListType === (_crd && TreeListType === void 0 ? (_reportPossibleCrUseOfTreeListType({
            error: Error()
        ...
        performInitialTreeSearch() {
          ...
          var nearbyTrees = this.getAvailableTrees(heroPosition, attackRange); //console.log(`[Lumberjack] 攻击范围内找到 ${nearbyTrees.length} 棵可用树木`);
          ...
          var searchRangeTrees = this.getAvailableTrees(heroPosition, this.TREE_SEARCH_RANGE); //console.log(`[Lumberjack] 搜索范围(${this.TREE_SEARCH_RANGE})内找到 ${searchRangeTrees.length} 棵可用树木`);

          if (searchRangeTrees.length > 0) {
            // 找到最近的树木并移动过去
            var nearestTree = searchRangeTrees[0];
            var nearestDistance = Vec3.distance(heroPosition, nearestTree.getWorldPosition());

            for (var i = 1; i < searchRangeTrees.length; i++) {
              var tree = searchRangeTrees[i];
              var distance = Vec3.distance(heroPosition, tree.getWorldPosition());
            ...
            var treePos = nearestTree.getWorldPosition();
            var targetPos = this.calculateTargetPosition(treePos, heroPosition); //console.log(`[Lumberjack] 移动到最近的${listName}树木，距离: ${nearestDistance.toFixed(2)}`);
        ...
        calculateTargetPosition(treePos, heroPos) {
          var attackRange = this.attackComponent.attackRangeValue;
          var desiredDistance = attackRange * this.TREE_DISTANCE_RATIO; // 计算从英雄到树木的方向向量
        ...
        onInitialMoveComplete() {
          ...
          var heroPosition = this.node.getWorldPosition();
          var attackRange = this.attackComponent.attackRangeValue;
          var nearbyTrees = this.getAvailableTrees(heroPosition, attackRange); //console.log(`[Lumberjack] 当前位置攻击范围内有 ${nearbyTrees.length} 棵可用树木`);
