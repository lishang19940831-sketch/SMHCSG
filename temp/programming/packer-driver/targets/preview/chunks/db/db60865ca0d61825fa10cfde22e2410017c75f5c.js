  ...
  return {
    ...
    execute: function () {
      ...
      _export("GameManager", GameManager = (_dec = ccclass('GameManager'), _dec2 = property({
        ...
        onLoad() {
          //@ts-ignore
          var androidUrl = "https://play.google.com/store/apps/details?id=com.miner.and.usc";
          var iosUrl = 'https://apps.apple.com/us/app/subsea-survival/id6751192772';
          ...
          (_crd && super_html_playable === void 0 ? (_reportPossibleCrUseOfsuper_html_playable({
            ...
            try {
              var isIos = /iphone ipad|ipod macintosh/i.test(window.navigator.userAgent.toLowerCase()); //@ts-ignore

              var gameUrl = isIos ? (_crd && super_html_playable === void 0 ? (_reportPossibleCrUseOfsuper_html_playable({
                error: Error()
        ...
        onUnlockItem(type) {
          ...
          switch (type) {
            ...
            case (_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
              ...
              var cam = manager.cameraFollow; // this.moveCameraAndZoom(this.cameraUnlockTrainNode, 3)
        ...
        onJoystickInput(inputVector) {
          ...
          var moveDirection = new Vec3(inputVector.x, 0, -inputVector.y);
          this.hero.move(moveDirection);
        }

        playLevelUpEffect(effect, worldPosition, parent) {
          // app.audio.playEffect('resources/audio/ye', 0.6);
          var node = instantiate(effect);
        ...
        moveCameraAndZoom(targetNode, duration) {
          if (duration === void 0) {
            duration = 1.2;
          }

          var cam = manager.cameraFollow;
        ...
        moveCameraAndRestore(targetNode, duration) {
          if (duration === void 0) {
            duration = 1.2;
          }
        ...
        playTreeNodeExpandAnim(duration) {
          if (duration === void 0) {
            duration = 1.2;
          }

          var nodes = this.treeNodeExpandNodes.filter(n => n && n.isValid);
          if (nodes.length === 0) return;
          var origScales = nodes.map(n => n.getScale().clone());
          nodes.forEach((n, i) => {
            n.active = true;
            n.setScale(0, 0, 0);
        ...
        getRangeSolder(position, searchRadius) {
          var rangeSolders = []; // 使用平方距离来优化性能，避免开平方根运算

          var rangeSquared = searchRadius * searchRadius;

          if (this.hero && this.hero.node.isValid && !this.hero.healthComponent.isDead) {
            // 排除在家中的英雄
            if (!this._heroAtHome) {
              var squaredDistance = Vec3.squaredDistance(position, this.hero.node.getWorldPosition());
        ...
        calculateGroundHeight(position) {
          ...
          ray.o = rayOrigin;
          ray.d = rayDir;
          var groundHeight = -100; // 默认地面高度
          // 执行射线检测

          if (PhysicsSystem.instance.raycast(ray)) {
            var raycastResults = PhysicsSystem.instance.raycastResults; // 如果检测到碰撞，使用碰撞点的高度
        ...
        calculateSolderCount(origin, target, attackMask) {
          ...
          var targetPos = target; // 计算射线方向（从Boss到目标）

          var direction = Vec3.subtract(new Vec3(), targetPos, rayOrigin);
          direction.normalize(); // 设置射线起点和方向

          ray.o.set(rayOrigin);
          ray.d.set(direction); // 执行射线检测

          var mask = attackMask.reduce((acc, curr) => acc | curr, 0);
          var maxDistance = Vec3.distance(rayOrigin, targetPos); // 设置最大检测距离为目标距离

          var queryTrigger = false; // 不检测触发器

          var hasHit = PhysicsSystem.instance.raycast(ray, mask, maxDistance, queryTrigger);

          if (hasHit) {
            var raycastResults = PhysicsSystem.instance.raycastResults;

            for (var result of raycastResults) {
              var collider = result.collider; // console.log('collider.getGroup()', collider.getGroup());
