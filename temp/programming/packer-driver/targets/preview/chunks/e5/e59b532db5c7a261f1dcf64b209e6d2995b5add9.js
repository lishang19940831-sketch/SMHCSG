      ...
      _export("Tree", Tree = (_dec = ccclass('Tree'), _dec2 = property({
        ...
        */
        ...
        createFragments() {
          ...
          var centerPos = this.node.worldPosition.clone(); // this.node.getChildByName("eff").active = true;
          // 生成多个碎片

          var _loop = function _loop() {
            // 克隆第一个有 MeshRenderer 的节点
            var fragment = instantiate(meshNodes[0]); // 设置为场景根节点的子节点（独立于草节点）

            fragment.setParent(_this.node.parent);
            fragment.setWorldPosition(centerPos); // 缩小碎片（在区间内随机）

            var randomScale = _this.fragmentScaleMin + Math.random() * (_this.fragmentScaleMax - _this.fragmentScaleMin);

            var originalFragmentScale = fragment.scale.clone();
            ...
            var angle = Math.PI * 2 * i / _this.fragmentCount + (Math.random() - 0.5) * 0.5;
            var distance = _this.fragmentScatterDistance * (0.7 + Math.random() * 0.6);
            var targetPos = new Vec3(centerPos.x + Math.cos(angle) * distance, centerPos.y + _this.fragmentUpHeight * (0.8 + Math.random() * 0.4), // 随机上升高度
            centerPos.z + Math.sin(angle) * distance); // 随机旋转

            var randomRotation = new Vec3(Math.random() * 360, Math.random() * 360, Math.random() * 360); // 碎片飞散动画（抛物线 + 旋转 + 缩小消失）

            tween(fragment) // 飞散阶段
            .to(_this.fragmentDuration, {
              worldPosition: targetPos // eulerAngles: randomRotation

            }, {
              easing: 'quadOut'
            }) // 同时缩小消失
            .to(_this.fragmentDuration * 0.5, {
            ...
            var fallStartTime = _this.fragmentDuration * 0.5; // 到达顶点后开始下落

            _this.scheduleOnce(() => {
              if (fragment && fragment.isValid) {
                var fallTargetPos = new Vec3(targetPos.x, centerPos.y - 0.5, targetPos.z);
                tween(fragment).to(_this.fragmentDuration * 0.5, {
        ...
        dropWood(damageData) {
          var treePosition = this.node.getWorldPosition();
          var dropPosition = new Vec3(treePosition.x, treePosition.y + 2, // 稍微抬高，让木头有掉落效果
          treePosition.z);
          this.schedule(() => {
            var rotation = new Quat();
            Quat.fromEuler(rotation, 0, Math.random() * 360, 0); // 从对象池获取木头节点

            var woodNode = manager.pool.getNode((_crd && ObjectType === void 0 ? (_reportPossibleCrUseOfObjectType({
              error: Error()
            ...
            var woodComponent = woodNode.getComponent(_crd && DropItemWood === void 0 ? (_reportPossibleCrUseOfDropItemWood({
              error: Error()
            ...
            var Character = damageData.damageSource.getComponent(_crd && Hero === void 0 ? (_reportPossibleCrUseOfHero({
              error: Error()
