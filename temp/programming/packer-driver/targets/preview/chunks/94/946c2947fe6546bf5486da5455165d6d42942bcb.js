    ...
    execute: function () {
      ...
      _export("FrameAnimationComponent", FrameAnimationComponent = (_dec = ccclass('FrameAnimationComponent'), _dec2 = property({
        ...
        onSlowStart(slowRatio, slowDuration) {
          if (this.animation) {
            // 对于帧动画，减速通过降低动画速度实现
            var state = this.animation.getState(this.currentAnimName);
        ...
        applyColorToAnimation(type) {
          ...
          var renderComp = this.animationNode.getComponent(Sprite);
        ...
        playAnimation(name, loop, timeScale, isShowLight) {
          if (timeScale === void 0) {
            timeScale = 1;
          }

          if (isShowLight === void 0) {
            isShowLight = false;
          }
        ...
        getAnimationDuration(animName) {
          ...
          var duration = 0;
          var clipName = animName || this.currentAnimName;

          if (clipName) {
            var state = this.animation.getState(clipName);
            duration = state ? state.duration : 0;
          } else {
            var currentState = this.animation.getState(this.currentAnimName);
        ...
        playAnimationFromProgress(name, loop, timeScale, startProgress) {
          if (timeScale === void 0) {
            timeScale = 1;
          }
