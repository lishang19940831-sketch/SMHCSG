      ...
      _export("BuildingSalesman", BuildingSalesman = (_dec = ccclass('BuildingSalesman'), _dec2 = property({
        ...
        showlock() {
          ...
          return _asyncToGenerator(function* () {
            if (_this.model) {
              _this.model.active = false;
            }

            if (_this.salesman) {
              _this.salesman.node.active = false;

              _this.salesman.node.setScale(0.1, 0.1, 0.1);

              _this.salesman.node.setWorldPosition(_this.node.getWorldPosition());

              _this.salesman.reset();
            }
          })();
        }
        /**
        ...
        showUnlockAnim() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              // 显示建筑模型并从 0 缩放弹出
              if (_this2.model) {
                _this2.model.active = true;

                _this2.model.setScale(new Vec3(0, 0, 0));
              } // 显示售货员并从小缩放弹出


              if (_this2.salesman) {
                _this2.salesman.node.active = true;

                _this2.salesman.node.setScale(0.1, 0.1, 0.1);
              }

              var modelTargetScale = _this2.model ? _this2.model.getScale().clone() : new Vec3(1, 1, 1); // 如果模型初始缩放已被设为0，目标用 (1,1,1)
              ...
              if (_this2.model) {
                tween(_this2.model).to(0.5, {
                  ...
                  // 建筑弹出完成后，再弹出售货员
                  if (_this2.salesman) {
                    tween(_this2.salesman.node).to(0.5, {
                      scale: v3(1, 1, 1)
                    }, {
                      easing: easing.backOut
                    }).start();
        ...
        onUnlockFinished() {
          ...
          var finalPos = this.workStationPos ? this.workStationPos.getWorldPosition() : undefined;
