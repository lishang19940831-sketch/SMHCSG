    ...
    execute: function () {
      ...
      _export("LosePanel", LosePanel = (_dec = ccclass('LosePanel'), _dec2 = property({
        ...
        showLoseAni() {
          ...
          var blackBgOpacity = this.blackBg.getComponent(UIOpacity) || this.blackBg.addComponent(UIOpacity);
          blackBgOpacity.opacity = 0;
          this.icon.scale = new Vec3(0.2, 0.2, 1);
          var loseTitlePos = this.loseTitle.position.clone();
          this.loseTitle.position = new Vec3(loseTitlePos.x, loseTitlePos.y + 150, loseTitlePos.z);
          var loseTitleOpacity = this.loseTitle.getComponent(UIOpacity) || this.loseTitle.addComponent(UIOpacity);
          loseTitleOpacity.opacity = 0;
          var downloadBtnPos = this.downloadBtn.position.clone();
          this.downloadBtn.position = new Vec3(downloadBtnPos.x, downloadBtnPos.y - 100, downloadBtnPos.z);
          var downloadBtnOpacity = this.downloadBtn.getComponent(UIOpacity) || this.downloadBtn.addComponent(UIOpacity);
          downloadBtnOpacity.opacity = 0; // 重试按钮初始状态

          var retryBtnPos = this.retryBtn.position.clone();
          this.retryBtn.position = new Vec3(retryBtnPos.x, retryBtnPos.y - 100, retryBtnPos.z);
          var retryBtnOpacity = this.retryBtn.getComponent(UIOpacity) || this.retryBtn.addComponent(UIOpacity);
        ...
        hideLoseAni() {
          ...
          var blackBgOpacity = this.blackBg.getComponent(UIOpacity);
          var loseTitleOpacity = this.loseTitle.getComponent(UIOpacity);
          var downloadBtnOpacity = this.downloadBtn.getComponent(UIOpacity);
          var retryBtnOpacity = this.retryBtn.getComponent(UIOpacity);
