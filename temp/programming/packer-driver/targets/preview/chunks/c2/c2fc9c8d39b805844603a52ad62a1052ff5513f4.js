  ...
  return {
    ...
    execute: function () {
      ...
      DataMgr = class DataMgr {
         ...
         */
        setData(key, value) {
          var stringValue = typeof value === "object" ? (_crd && Json === void 0 ? (_reportPossibleCrUseOfJson({
            error: Error()
          }), Json) : Json).stringify(value) : value.toString();
          var encryptedValue = (_crd && Crypt === void 0 ? (_reportPossibleCrUseOfCrypt({
            error: Error()
        ...
        getText(key) {
          try {
            var encryptedValue = sys.localStorage.getItem(key);
        ...
        getNumber(key) {
          var textValue = this.getText(key);

          if (textValue) {
            var numberValue = Number(textValue);
