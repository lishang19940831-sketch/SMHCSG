      ...
      _export("HealthBar_Cycle", HealthBar_Cycle = (_dec = ccclass('HealthBar_Cycle'), _dec2 = property({
        ...
        update(dt) {
          ...
          if (this.isAnimating) {
            ...
            var progress = Math.min(1.0, this.animationTimer / this.animationDuration); // 如果背景填充存在，平滑更新其填充范围

            if (this.backgroundFill) {
              // 计算当前应该显示的血量百分比（线性插值）
              var currentFillRange = this.currentHealthPercent - (this.currentHealthPercent - this.targetHealthPercent) * progress;
        ...
        updateHealth(percentage) {
          ...
          var healthPercent = Math.max(0, Math.min(1, percentage)); // 更新填充区域范围（立即更新）
