    ...
    execute: function () {
      ...
      _export("Train", Train = (_dec = ccclass('Train'), _dec2 = property({
        ...
        update(dt) {
          ...
          var headPos = this.track.getPositionAt(this._progress);
          var headDir = this.track.getDirectionAt(this._progress);
          ...
          var stationPos = this.track.getPositionAt(this._stationProgress);
          var distToStation = Vec3.distance(headPos, stationPos);
        ...
        alignFacingAtProgress(progress) {
          ...
          var dir = this.track.getDirectionAt(progress);
        ...
        initAtProgress(progress) {
          ...
          var stationPos = this.track.getPositionAt(this._stationProgress);
          var headPos = this.track.getPositionAt(progress);
        ...
        setVisible(visible) {
          ...
          if (visible) {
            ...
            for (var _car of this.trainCars) {
              var _car$node2;

              if (_car != null && (_car$node2 = _car.node) != null && _car$node2.isValid) _car.node.active = false;
            }
        ...
        _doUnload(delay) {
          ...
          var execute = () => {
            ...
            var rfm = (_crd && ResourceFieldManager === void 0 ? (_reportPossibleCrUseOfResourceFieldManager({
              error: Error()
            }), ResourceFieldManager) : ResourceFieldManager).instance;
            console.log("[Train] _doUnload execute: autoRun=" + this.autoRun + ", RFM\u5B58\u5728=" + !!rfm);
            var harvestedCrops = rfm ? rfm.collectHarvestedCrops() : [];
        ...
        _collectPendingItems() {
          ...
          for (var car of this._allCars) {
            ...
            var itemNodes = car.itemLayout.getAllItemNodes().filter(n => n && n.isValid);
            ...
            var resourceType = car.resourceType; // 将车厢内所有物品节点安全回收到对象池
            // 不做节点迁移，直接 putNode，由发送阶段让仓库从对象池取新节点飞入

            for (var n of itemNodes) {
          ...
          var totalCount = this._pendingItems.reduce((s, it) => s + it.count, 0);
        ...
        _flushPendingItems(onAllArrived) {
          ...
          var fromPosition = this.node.getWorldPosition();
          var remaining = 0;

          for (var item of this._pendingItems) {
            var _this$unloadManager$g, _this$unloadManager;

            var container = (_this$unloadManager$g = (_this$unloadManager = this.unloadManager) == null ? void 0 : _this$unloadManager.getContainer(item.type)) != null ? _this$unloadManager$g : null;
            ...
            for (var i = 0; i < item.count; i++) {
              var accepted = container.receive(fromPosition, () => {
                remaining--;
        ...
        _updateCars(updateFacing) {
          if (updateFacing === void 0) {
            updateFacing = true;
          }
        ...
        _updateHeadFacing(direction) {
          ...
          var yaw = Math.atan2(direction.x, direction.z) * (180 / Math.PI) + this.yawOffset;
