    ...
    execute: function () {
      ...
      _export("Chainsaw", Chainsaw = (_dec = ccclass('Chainsaw'), _dec2 = property({
        ...
        update(_dt) {
          ...
          var anchorNode = this.sawAnchor && this.sawAnchor.isValid ? this.sawAnchor : this.node; // 只在火车行驶时检测
        ...
        _doHarvest() {
          var fieldMgr = (_crd && ResourceFieldManager === void 0 ? (_reportPossibleCrUseOfResourceFieldManager({
            error: Error()
          ...
          var hits = fieldMgr.queryInStrip(this._prevPos, this._currPos, this.leftWidth, this.rightWidth); // 调试可视化：无论有无命中都绘制条带
          ...
          for (var hit of hits) {
            ...
            var targetCar = this.trainManager.getCarForResource(hit.type);
            ...
            var reserve = targetCar.reserveSlot(); // ④ 向车厢注入资源计数

            var added = this.trainManager.addResource(result.type, result.amount);
        ...
        _spawnAndFlyToCar(type, fromWorldPos, targetCar, targetPos, initialWorldRot, reservedLpos) {
          var itemNode = manager.pool.getNode(type);
          if (!itemNode) return;
          ...
          var startPos = new Vec3(fromWorldPos.x, fromWorldPos.y + this.flyStartYOffset, fromWorldPos.z); // 每帧实时更新的目标坐标（Vec3 复用，减少 GC）

          var liveTarget = targetPos.clone();
          var progressObj = {
            progress: 0
          };
          tween(progressObj).to(this.flyDuration, {
            ...
            onUpdate: () => {
              ...
              var t = progressObj.progress; // 实时根据预留格子计算世界坐标（随车厢移动）

              if (reservedLpos && targetCar.itemLayout) {
                var pos = targetCar.itemLayout.getItemPosition(reservedLpos);
                if (pos) liveTarget.set(pos);
              } // XZ 线性插值


              var x = startPos.x + (liveTarget.x - startPos.x) * t;
              var z = startPos.z + (liveTarget.z - startPos.z) * t; // Y：线性插值 + 抛物线高度偏移

              var y = startPos.y + (liveTarget.y - startPos.y) * t + 4 * this.flyArcHeight * t * (1 - t);
        ...
        static _makeCrossMesh(center, armLen, c) {
          ...
          var positions = [x - A - H, y, z - A + H, x - A + H, y, z - A - H, x + A + H, y, z + A - H, x + A - H, y, z + A + H, // 第二臂 (x+z) 方向
          x - A - H, y, z + A - H, x - A + H, y, z + A + H, x + A + H, y, z - A + H, x + A - H, y, z - A - H];
          var colors = [];

          for (var i = 0; i < 8; i++) colors.push(cr, cg, cb, ca);
        ...
        _drawStrip(prev, curr, hitPoses) {
          ...
          var abx = curr.x - prev.x;
          var abz = curr.z - prev.z;
          var abLen = Math.sqrt(abx * abx + abz * abz);
          ...
          var dx = abx / abLen;
          var dz = abz / abLen;
          var nx = dz; // 单位法向量（右侧为正）

          var nz = -dx;
          var Y = Math.max(prev.y, curr.y) + 0.15; // 略高于地面
          // 条带四个角点（世界坐标，节点本身在世界原点不旋转）
          // p0=prevLeft, p1=prevRight, p2=currRight, p3=currLeft

          var p0 = new Vec3(prev.x - nx * this.leftWidth, Y, prev.z - nz * this.leftWidth);
          var p1 = new Vec3(prev.x + nx * this.rightWidth, Y, prev.z + nz * this.rightWidth);
          var p2 = new Vec3(curr.x + nx * this.rightWidth, Y, curr.z + nz * this.rightWidth);
          var p3 = new Vec3(curr.x - nx * this.leftWidth, Y, curr.z - nz * this.leftWidth); // 将 _stripNode 放到世界原点（scale=1, rot=identity），mesh 直接用世界坐标顶点
          ...
          for (var i = 0; i < hitPoses.length; i++) {
            var [hn, hmr] = this._getOrCreateHitNode(i);
          ...
          for (var _i = hitPoses.length; _i < this._hitNodes.length; _i++) {
            this._hitNodes[_i].active = false;
          }
        ...
        _onTrainArrived() {
          ...
          if (this._flyingNodes.size > 0) {
            ...
            for (var node of this._flyingNodes) {
