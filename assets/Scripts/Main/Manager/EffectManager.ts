...
export class EffectManager extends Component {
     ...
     * @returns 飞行的节点
     */
    public flyNodeInParabola(params: IParabolaFlyParams): Node {
        const { node, target, callback, duration = 0.3, height = 3, targetWorldScale, targetWorldRotation } = params;
       ...
       //console.warn('目标坐标无效，无法执行抛物线飞行');
        ...
        const targetPos = isNodeTarget ? target.getWorldPosition() : target;
        
        // 获取起始缩放和旋转
        const startScale = node.getWorldScale();
        const startRotation = node.getWorldRotation();
                    ...
                    if (targetWorldScale || isNodeTarget) {
                        ...
                        if (finalTargetScale) {
                            const currentScale = new Vec3(
                                startScale.x + (finalTargetScale.x - startScale.x) * ratio,
                                startScale.y + (finalTargetScale.y - startScale.y) * ratio,
                                startScale.z + (finalTargetScale.z - startScale.z) * ratio
                            );
