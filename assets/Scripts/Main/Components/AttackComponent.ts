...
import { CharacterState } from '../../Main/Common/CommonEnum';
...
export class AttackComponent extends BaseComponet {
    ...
    /** 剩余攻击冷却时间 */
    private _remainingCooldownTime: number = 0;
    /** 是否已执行本次攻击的伤害 */
    private hasPerformedAttack: boolean = false;
    /** 攻击状态标志 */
    private _canPerformAttack: boolean = true;
    /** 记录冷却时间的超出部分，用于抵消下一轮冷却 */
    private cooldownCarry: number = 0;
     ...
     * 获取伤害浮动范围
     */
    public get damageRangeValue(): number {
    ...
    protected handleAttackCooldown(dt: number) {
        ...
        if (!this._canPerformAttack && this._remainingCooldownTime > 0) {
            this._remainingCooldownTime -= dt;
            ...
            if (this._remainingCooldownTime <= 0) {
                ...
                // 发送攻击冷却结束事件
                this.node.emit(ComponentEvent.ATTACK_COOLDOWN_END);
            }
        }
    }
    ...
    public updateAttackTarget( target : Node ) {
        ...
        direction.normalize();
        this.node.emit(ComponentEvent.SET_FACE_DIRECTION_FROM_3D, direction, 1);
    }
