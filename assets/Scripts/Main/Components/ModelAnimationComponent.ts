...
export class ModelAnimationComponent extends BaseAnimationComponent {
    ...
    /** 默认骨骼颜色 */
    protected defaultColor: Color = new Color(0, 0, 0, 255);
    /** 受伤效果颜色 (红色) */
    protected readonly hurtColor: Color = new Color(200, 0, 0, 255);
    ...
    protected applyColorToAnimation(type: ColorEffectType) {
        ...
        switch (type) {
            ...
            case ColorEffectType.SLOW:
                ...
                break;
            case ColorEffectType.ELECTRIC_SHOCK:
     ...
     * 播放移动动画
     */
    public playMove(timeScale?: number) {
    ...
    public playDissolve() {
        ...
        // this.modelNode.getComponent(Dissolve)?.DissolveUp();
     ...
     */
    public playAnimation(name: string, loop: boolean, timeScale: number = 1, crossFade: number = 0.1) {
        if (!this.animation) {
     ...
     */
    public playAnimationFromProgress(name: string, loop: boolean, timeScale: number = 1, startProgress: number = 0, crossFade: number = 0.1) {
        if (!this.animation) return;
