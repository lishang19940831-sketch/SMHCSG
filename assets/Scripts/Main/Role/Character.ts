...
import { CharacterState, FrameEventId } from '../../Main/Common/CommonEnum';
...
export abstract class Character extends Component {
    ...
    /** 是否支持移动攻击 */
    protected supportMoveAttack: boolean = false;
    ...
    protected startAttackAnimation() {
        // 判断是否应该播放移动攻击动画
        const isMoving = this.movementComponent.isMoving;
        const shouldPlayRunAttack = this.supportMoveAttack && isMoving;
        
        // 选择动画名称
        const animName = shouldPlayRunAttack ? 
            this.animationComponent.RunAttackAnimName : 
            this.animationComponent.AttackAnimName;
        
        // 计算攻击动画的timeScale
        const attackTimeScale = this.calculateAttackTimeScale(animName);
        
        // console.log(`[攻击动画] 开始播放攻击动画`, {
        //     '角色': this.node.name,
        //     '动画名称': animName,
        ...
        const eventId = shouldPlayRunAttack ? 
            FrameEventId.RUN_ATTACK_DAMAGE : 
        ...
        // 检查动画是否成功播放
        this.scheduleOnce(() => {
            const isPlaying = this.animationComponent.isPlayingAnimation();
            const currentAnimName = this.animationComponent.getCurrentAnimationName();
            const progress = this.animationComponent.getCurrentAnimationProgress();
            
            // console.log(`[攻击动画] 动画播放状态检查`, {
            //     '角色': this.node.name,
            //     '是否正在播放': isPlaying,
    ...
    protected calculateAttackTimeScale(attAnimName:string): number {
        ...
        const cooldownTime = this.attackComponent.attackCooldownTime;
     ...
     * 处理攻击完成后的逻辑
     */
    protected handleAttackComplete() {
     ...
     * 处理移动攻击动画切换
     */
    protected handleMoveAttackAnimation() {
        const isMoving = this.movementComponent.isMoving;
        const currentAnimName = this.animationComponent.getCurrentAnimationName();
        const isPlayingRunAttack = currentAnimName === this.animationComponent.RunAttackAnimName;
        const isPlayingAttack = currentAnimName === this.animationComponent.AttackAnimName;
     ...
     * 切换到移动攻击动画
     */
    protected switchToRunAttack() {
        // 获取当前动画进度
        const currentProgress = this.animationComponent.getCurrentAnimationProgress();
        
        // 计算新动画的timeScale
        const attackTimeScale = this.calculateAttackTimeScale(this.animationComponent.RunAttackAnimName);
        
        // 清除旧的帧事件
        this.animationComponent.clearFrameEvents(false);
    ...
    protected onPerformAttack(damageData: DamageData) {
        // console.log("[Character] onPerformAttack", damageData,this.node.name);
        // 子类根据需要实现攻击逻辑
    }
    ...
    protected onMoveStateUpdate(isMoving: boolean) {
        ...
        if (this.supportMoveAttack && currentState === CharacterState.Attack) {
    ...
    public moveWorld(direction: Vec3) {
        ...
        // 发送世界坐标移动事件
        this.node.emit(ComponentEvent.CHARACTER_MOVE_WORLD, direction);
    }
