...
class AudioMgr {
    ...
    /** 音效播放记录，用于防重复播放 */
    private effectPlayRecord: Map<string, number> = new Map();

    /** 音效防重复播放间隔时间（毫秒），默认200ms */
    private effectCooldownTime: number = 100;
     ...
     */
    private isEffectInCooldown(path: string): boolean {
     ...
     */
    public setEffectCooldownTime(cooldownTime: number): void {
     ...
     */
    public getEffectCooldownTime(): number {
     ...
     */
    public clearEffectPlayRecord(path?: string): void {
        if (path) {
